...
while true; do
  ...
  # 1) Resolve deterministic Explore state. Existing verified families always
  # have priority. The router may consume the latest durable AI proposal receipt
  # produced asynchronously by GitHub Actions, but it never calls a model or the
  # network and AI never receives selection/promotion/execution authority.
  ...
  # 5) Advance cumulative Exploit improvement after the cycle receipt is durable.
