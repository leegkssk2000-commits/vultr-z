...
def pool_tick(
    ...
    *,
    ...
    if isinstance(previous_pool, Mapping) and previous_pool.get("schema_version") == SCHEMA:
        previous_active = [str(v.get("alpha_id")) for v in previous_pool.get("active") or [] if isinstance(v, Mapping)]
        previous_reserve = [str(v.get("alpha_id")) for v in previous_pool.get("reserve") or [] if isinstance(v, Mapping)]
        previous_target = str(previous_pool.get("state") or "") == "PASS_SURVIVOR_POOL_TARGET_3_PLUS_2"
    current_active = [str(v.get("alpha_id")) for v in active]
    current_reserve = [str(v.get("alpha_id")) for v in reserve]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="ZEL deterministic multi-survivor 3+2 pool")
