...
def _features(rs:list[dict[str,float]])->dict[str,list[Any]]:
    ...
    ema20=_ema(close,20); ema50=_ema(close,50); sma_vol20=_sma(volume,20); std20=_std(close,20)
    highest20=[]; lowest20=[]
    for i in range(len(rs)):
        w=rs[max(0,i-19):i+1]
        highest20.append(max(x["high"] for x in w)); lowest20.append(min(x["low"] for x in w))
    return {"ema20":ema20,"ema50":ema50,"sma_vol20":sma_vol20,"std20":std20,"highest20":highest20,"lowest20":lowest20}


def _events(rs:list[dict[str,float]], f:dict[str,list[Any]])->list[dict[str,Any]]:
    out=[]
    for i in range(50,len(rs)-max(HORIZONS)-1):
        r=rs[i]; prev=rs[i-1]
        vol_ma=f["sma_vol20"][i]; sd=f["std20"][i]; ema20=f["ema20"][i]; ema50=f["ema50"][i]
        if not vol_ma or not sd or r["close"]<=0: continue
        vol_ratio=r["volume"]/vol_ma if vol_ma>0 else 0.0
        z=(r["close"]-ema20)/sd if sd>0 else 0.0
        body=abs(r["close"]-r["open"])/r["close"]
        range_pct=(r["high"]-r["low"])/r["close"]
        ret1=r["close"]/prev["close"]-1.0 if prev["close"] else 0.0
        # Fixed, preregistered-like primitive library. No threshold sweep.
        defs=[
            ...
            ("P_TREND_PULL_LONG", ema20>ema50 and r["close"]>ema20 and ret1>0, 1),
            ("P_TREND_PULL_SHORT", ema20<ema50 and r["close"]<ema20 and ret1<0, -1),
            ("P_ZREV_LONG", z<=-1.5, 1),
            ("P_ZREV_SHORT", z>=1.5, -1),
            ("P_WIDE_BODY_CONT_LONG", body>=0.004 and range_pct>=0.006 and r["close"]>r["open"], 1),
            ("P_WIDE_BODY_CONT_SHORT", body>=0.004 and range_pct>=0.006 and r["close"]<r["open"], -1),
        ]
    ...
    for interval in INTERVALS:
        ...
        for symbol in SYMBOLS:
            ...
            for ev in _events(rs,f):
                i=int(ev["i"]); side=int(ev["side"]); pid=str(ev["primitive_id"])
                entry=rs[i+1]["open"]
                for h in HORIZONS:
                    exit_px=rs[i+1+h]["close"]
                    gross=(exit_px/entry-1.0)*10000*side
                    buckets.setdefault((interval,pid,h),[]).append(gross)
    rows=[]
    for (interval,pid,h),gross in buckets.items():
        net=[x-COST_BPS for x in gross]; n=len(net)
        ...
        rows.append({
            ...
            "profit_factor":_pf(net),"win_rate":sum(1 for x in net if x>0)/n if n else None,
            "drawdown_bps":_dd(net),"cost_bps_per_trade":COST_BPS,
            "economically_usable":bool(n>=MIN_EVENTS and (ne or 0)>0 and (_pf(net) or 0)>1.0),
        })
    rows.sort(key=lambda x:(not x["economically_usable"],-(x["net_expectancy_bps"] or -1e9),-x["events"]))
    usable=[x for x in rows if x["economically_usable"]]
    return {
        "schema_version":"zel.a1_alpha_primitive_miner.v1","boundary":BOUNDARY,"development_only":True,
        ...
        "primitive_count":len(rows),"economically_usable_count":len(usable),"usable":usable[:20],"rows":rows,
...
def compact(result:dict[str,Any])->list[dict[str,Any]]:
    return [{k:r.get(k) for k in ("primitive_id","interval","horizon_bars","events","gross_expectancy_bps","net_expectancy_bps","profit_factor","win_rate","drawdown_bps","cost_bps_per_trade")} for r in (result.get("usable") or [])]
