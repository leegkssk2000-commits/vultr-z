...
LOCK_ACT_BPS=300.0*math.sqrt(4/24)
LOCK_GROSS_BPS=14.0
...
def dd(xs):
 ...
 return m
def metrics_from_gross(g,cost=COST_BPS):
...
def exit_gross(rs,ei,xi,side,profit_lock=False):
 ep=float(rs[ei]['open']); activated=False
 for j in range(ei,xi+1):
  if activated:
   floor=ep*(1+LOCK_GROSS_BPS/10000) if side=='long' else ep*(1-LOCK_GROSS_BPS/10000)
   if side=='long' and float(rs[j]['low'])<=floor:
    px=min(floor,float(rs[j]['open'])); return (px/ep-1)*10000,j
   if side=='short' and float(rs[j]['high'])>=floor:
    px=max(floor,float(rs[j]['open'])); return (1-px/ep)*10000,j
  if profit_lock:
   fav=(float(rs[j]['high'])/ep-1)*10000 if side=='long' else (1-float(rs[j]['low'])/ep)*10000
   if fav>=LOCK_ACT_BPS: activated=True
 xp=float(rs[xi]['close']); return (xp/ep-1)*10000*(1 if side=='long' else -1),xi
def signal(mode,rs,i):
 c=float(rs[i]['close']); o=float(rs[i]['open']); s50=sma(rs,i)
 if mode=='breakout':
  p=rs[i-LOOKBACK:i]; hi=max(float(x['high']) for x in p); lo=min(float(x['low']) for x in p)
  if c>hi and c>s50:return 'long'
  if c<lo and c<s50:return 'short'
  return None
 if mode=='vol_cont':
  ...
  if a>0 and tr>=1.5*a:
   if c>o and c>s50:return 'long'
   if c<o and c<s50:return 'short'
  return None
 prev=float(rs[i-1]['close']); r=c/prev-1 if prev else 0.0
 if abs(r)<SCALED_SHOCK:return None
 if r<0:
  if mode in ('basis_hf_repaired2','basis_hf_lock') and c>=s50 and abs(c-prev)>=atr_prev(rs,i):return None
  return 'long'
 if r>0 and (mode=='basis_hf_raw' or c<s50):return 'short'
 return None
def run_one(mode):
 rows=[]; src={}
 for sym in SYMBOLS:
  ...
  while i<len(rs)-HOLD-1:
   ...
   ei=i+1; xi=ei+HOLD-1; gross,actual_xi=exit_gross(rs,ei,xi,side,mode=='basis_hf_lock')
   rows.append({'symbol':sym,'side':side,'gross_bps':gross}); i=actual_xi+1
 g=[x['gross_bps'] for x in rows]; m=metrics_from_gross(g)
 by_symbol={s:metrics_from_gross([x['gross_bps'] for x in rows if x['symbol']==s]) for s in SYMBOLS}
 by_side={s:metrics_from_gross([x['gross_bps'] for x in rows if x['side']==s]) for s in ('long','short') if any(x['side']==s for x in rows)}
 costs={str(c):metrics_from_gross(g,float(c)) for c in (14,28,40)}
...
def run():
 configs=[('breakout','newarch_4h_trend_breakout_v1','4H_20BAR_BREAKOUT_WITH_SMA50_REGIME'),('vol_cont','newarch_4h_vol_expansion_continuation_v1','4H_TR_GE_1P5_ATR14_SMA50_CONTINUATION'),('basis_hf_repaired2','basis_premium_collector_4h_scaled_proven_vetoes_v1','4H_TIME_SCALED_MEAN_REVERSION_PROVEN_VETOES'),('basis_hf_lock','basis_premium_collector_4h_scaled_profit_lock_v1','4H_TIME_SCALED_MEAN_REVERSION_PROVEN_VETOES_PLUS_SCALED_PROFIT_LOCK')]
 cand={}
 for mode,cid,arch in configs:
  cand[cid]={'architecture':arch,**run_one(mode)}
 r={'schema_version':'zel.a1_gen2_highfreq_dual_dev.v4','boundary':BOUNDARY,'development_only':True,'prospective':False,'uses_data_strictly_before_gen1_boundary':True,'parameter_sweep':False,'tuned_thresholds':0,'scaled_basis_shock_abs_return':SCALED_SHOCK,'scaled_profit_lock_activation_bps':LOCK_ACT_BPS,'profit_lock_floor_gross_bps':LOCK_GROSS_BPS,'hold_bars':HOLD,'candidates':cand,'selection_authority':False,'promotion_authority':False,'execution_authority':'NONE','order_authority':'BLOCKED','live_trade_authority':'BLOCKED'}
