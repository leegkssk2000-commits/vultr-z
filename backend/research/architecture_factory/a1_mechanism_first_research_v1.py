...
def _combined_evidence() -> dict[str, Any]:
    ...
    for raw in list(base.get("sources") or []) + list(supplement.get("sources") or []):
        ...
        if not sid or sid in seen:
            continue
        seen.add(sid)
        # Architecture Factory v1 consumes these generic evidence fields. Curated
        # supplement semantics remain hypothesis authority only; no numeric threshold
        # is imported from the source.
...
def _guard(candidate: Mapping[str, Any]) -> tuple[bool, list[str], str, list[str]]:
    ...
    try:
        ...
        multiple = 0.0
    # This is the already-frozen Architecture Factory design objective, not achieved
    # alpha evidence and not a promotion threshold.
    if multiple < 2.0:
