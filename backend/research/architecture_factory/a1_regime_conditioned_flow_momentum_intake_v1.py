...
def run(source_receipt_path, out_path):
    ...
    boundary_assigned=bool(prereg.get('fresh_prospective_boundary_utc'))
    out={'schema_version':'zel.a1.rcfm.intake_receipt.v1','state':'PASS_RCFM_H2_INTAKE_READY' if h2['state']=='PASS_ARCHETYPE_INTAKE' else 'HOLD_RCFM_H2_INTAKE','candidate_id':prereg['candidate_id'],'archetype_id':prereg['archetype_id'],'factory_run':prereg['factory_run'],'source_receipt':src,'h2_receipt':h2,'fresh_boundary_assigned':boundary_assigned,'next_permitted_action':'RUN_FROZEN_PROSPECTIVE_EVALUATOR' if h2['state']=='PASS_ARCHETYPE_INTAKE' and boundary_assigned else ('FREEZE_EVALUATOR_POLICY_THEN_ASSIGN_FRESH_PROSPECTIVE_BOUNDARY' if h2['state']=='PASS_ARCHETYPE_INTAKE' else 'HOLD'),'selection_authority':False,'promotion_authority':False,'execution_authority':'NONE','order_authority':'BLOCKED','live_trade_authority':'BLOCKED','exchange_order_submitted':False,'protected_mutations':0}
    ...
    print(json.dumps({'state':out['state'],'h2':h2['state'],'max_similarity':h2['maximum_structural_similarity'],'source_rows':src.get('history_rows'),'boundary_assigned':boundary_assigned},sort_keys=True))
    return 0 if h2['state']=='PASS_ARCHETYPE_INTAKE' else 2

def self_test():
    p=read(PREREG); assert p['candidate_id']=='NEW_RCFM_001'; assert p['mode']=='NEW_ARCHITECTURE'; assert p['baseline_mutated'] is False; assert p.get('fresh_prospective_boundary_utc') in (None,'2026-08-18T10:20:00Z'); assert len(p['external_evidence_ids'])>=1; print('PASS_RCFM_INTAKE_STATIC'); return 0
