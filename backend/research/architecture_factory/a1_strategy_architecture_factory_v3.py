...
def _review_gemini_candidate(c: Mapping[str, Any], work: Path, env: Mapping[str, str]) -> dict[str, Any]:
    ...
    for name, row in reviews.items():
        ...
        if row.get("successful") and decision == "REJECT":
            rejects += 1
    return {**c, "cross_reviews": reviews, "independent_passes": passes, "independent_rejects": rejects, "score": round(base_score(c) + passes * 2.5 - rejects * 4.0, 4)}
...
def run(output: Path) -> dict[str, Any]:
    ...
    gemini_state: dict[str, Any] = {"enabled": gemini_enabled, "activation_rule": "done_count>=25 AND GEMINI_API_KEY present AND GEMINI_ECONOMIC_REBUILD_ENABLED not false", "done_count": done_count, "purpose": "POST_25_ECONOMIC_REBUILD_ONLY", "generator": {"successful": False, "state": "DEFERRED_PRE25" if done_count < 25 else "KEY_OR_ENABLEMENT_MISSING"}, "critic_reviewed_count": 0}
    if gemini_enabled:
        ...
        context = {"objective": "post-25 economic rebuild: find robust realistic-cost Net+ mechanisms, not structural completeness", "verified_round_trip_cost_bps_reference": 14.0, "available_source_vocabulary": ["ohlcv", "volume", "funding", "basis", "open_interest", "l2_order_book", "trade_flow"], "current_failure_targets": targets, "external_evidence": source_rows, "constraints": {"baseline_mutation": False, "threshold_sweep": False, "best_horizon_cherry_pick": False, "fee_reduction": False, "sealed_holdout_visibility": False, "new_architecture_allowed": True, "economic_objective": "realistic-cost net profit first"}}
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="a1-gemini-candidates-") as td:
                ...
                for idx, candidate in enumerate(gemini_candidates):
                    work = root / str(idx); work.mkdir(); reviewed.append(_review_gemini_candidate(candidate, work, env))
        except Exception as exc:
            gemini_state["generator"] = {"successful": False, "state": "CALL_FAILED", "error": safe_error(exc)}
        reviewed = [candidate if candidate.get("provider") == "gemini" else _add_gemini_review(candidate) for candidate in reviewed]
        gemini_state["critic_reviewed_count"] = sum(1 for x in reviewed if (x.get("cross_reviews") or {}).get("gemini") is not None)
    ...
    result["state"] = "PASS_ARCHITECTURE_FACTORY_TOP3_READY_FOR_ALPHA_PROOF" if any(x.get("alpha_proof_candidate_ready") for x in top3) else "HOLD_ARCHITECTURE_FACTORY_NO_ALPHA_PROOF_CANDIDATE"
    result.pop("receipt_sha256", None); result["receipt_sha256"] = sha(result)
...
def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument("--output", type=Path, default=Path("out/a1_strategy_architecture_factory_v3.json")); ap.add_argument("--self-test", action="store_true"); args = ap.parse_args()
    if args.self_test: return self_test()
    result = run(args.output)
    print(json.dumps({"state": result["state"], "done_count": result.get("gemini", {}).get("done_count"), "gemini_enabled": result.get("gemini", {}).get("enabled"), "gemini_generator": result.get("gemini", {}).get("generator"), "alpha_proof_ready_count": result.get("alpha_proof_ready_count"), "top3": [{"id": x.get("candidate_id"), "provider": x.get("provider"), "family": x.get("architecture_family"), "score": x.get("score")} for x in result.get("top3") or []]}, ensure_ascii=False, sort_keys=True))
    return 0
