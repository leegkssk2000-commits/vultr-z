...
def fingerprint(sid: str, raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    cost = float(raw.get("verified_pretrade_cost_bps") or 14.0)
    if status == "A1_SPARSE_EVENT_FUTILITY":
        primary = "STRUCTURAL_EVENT_RATE_TOO_LOW"
        diagnosis = "Frozen mechanism exhausted its prospective resource budget with too few decisions; threshold loosening is forbidden."
    elif isinstance(gross, (int, float)) and float(gross) <= 0:
        primary = "NEGATIVE_GROSS_EDGE"
        diagnosis = "Frozen mechanism loses before costs; payer/direction/horizon thesis requires redesign rather than fee or threshold rescue."
    elif isinstance(gross, (int, float)) and float(gross) < cost:
        primary = "POSITIVE_GROSS_EDGE_BELOW_COST"
        diagnosis = "Frozen mechanism shows raw edge but natural move is below verified round-trip cost geometry."
    else:
        primary = "TERMINAL_ECONOMIC_OR_CAUSAL_FAIL"
        diagnosis = "Frozen mechanism failed its terminal gate and may only be tested through a fresh single-axis repair or distinct replacement architecture."
    return {
        "strategy_id": sid,
        ...
        "baseline": {
            ...
            "intents": int(raw.get("intent_count") or 0),
...
def prompt_for(fp: Mapping[str, Any], evidence: list[dict[str, Any]]) -> str:
    shape = {
        "candidates": [{
            ...
            "changed_axis": "exactly_one_axis",
            "mechanism": "why money exists/who pays",
            "payer": "participant/inefficiency",
            "entry_event": "entry-time observable event",
            "direction_rule": "long/short/both",
            "native_horizon": "natural holding horizon",
            "regime_owner": "when it should/should not trade",
            "invalidation": "causal invalidation",
            "exit_logic": "exit rationale without tuned thresholds",
            "time_stop_rationale": "why horizon fits mechanism",
            "turnover_cost_budget": "why expected move can dominate cost",
            "required_sources": ["ohlcv|volume|funding|basis|open_interest|l2_order_book|trade_flow"],
            "evidence_ids": ["F1"],
            "expected_move_cost_multiple_target": 2.0,
            "falsification": "bounded fresh prospective kill test",
            "forbidden_changes": ["fees", "threshold sweep", "best-horizon selection", "post-outcome loss deletion"],
            "why_distinct": "why this is not the frozen control or a duplicate filter",
        }]
    }
    return (
        "You are a research-only crypto strategy architect. For THIS terminal strategy return exactly 4 candidates: "
        "exactly 3 REPAIR candidates plus exactly 1 NEW_ARCHITECTURE replacement. Each REPAIR must change one and only one causal axis relative to the immutable baseline. "
        "The NEW_ARCHITECTURE must replace the payer/mechanism rather than stack another filter; still provide one architecture-defining changed_axis name for schema compatibility. "
        "Never loosen numeric thresholds, sweep parameters, reduce fees, delete losing trades, inspect future outcomes, or choose the best horizon after outcomes. "
        "Sparse-event failures must redesign the event/source/context or replace the architecture; do not simply make entry easier. "
        "Use only these native sources: ohlcv, volume, funding, basis, open_interest, l2_order_book, trade_flow. "
        "Prefer natural expected-move geometry plausibly >=2x the verified 14bps round-trip reference, but treat that only as a design target. "
        "Primary evidence must anchor the economic mechanism; community sources are hypothesis-only. Return JSON only. "
...
def review_candidates(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="a1-terminal-swarm-v2-") as td:
        ...
        for i,c in enumerate(candidates):
            ...
            source_ready=all(s in {"ohlcv","volume","funding","basis","open_interest","l2_order_book","trade_flow"} for s in (c.get("required_sources") or []))
            score=base_score(c)+passes*2.5-rejects*4.0
    ...
    out.sort(key=lambda x:(-float(x["score"]),x["candidate_id"]))
    return out
...
def run(output: Path) -> dict[str, Any]:
    ...
    reviewed=review_candidates(generated)
    by_strategy={}
    for fp in fps:
        sid=fp["strategy_id"]
    ...
    result={
        "schema_version":"zel.a1_terminal_repair_swarm.v2",
        ...
        "ledger_done_count":int(ledger.get("done_count") or 0),
        ...
        "eligible_count":sum(1 for x in reviewed if x.get("eligible_for_preregistration")),
        "dedup_cosine_threshold":0.85,
        "provider_state":providers,
        "strategies":by_strategy,
        "global_queue":reviewed,
        "launch":{"state":"DEFER_TO_GLOBAL_ONE_HEAVY_OWNER","candidate":None},
...
def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("--output",type=Path,default=Path("out/a1_terminal_repair_swarm_v2.json")); ap.add_argument("--self-test",action="store_true")
    args=ap.parse_args()
    if args.self_test:
        f=fingerprint("x",{"status":"A1_COST_FUTILITY","gross_expectancy_bps":2.0,"net_expectancy_bps":-10.0,"verified_pretrade_cost_bps":14.0,"receipt_sha":"r"})
        assert f["primary"]=="POSITIVE_GROSS_EDGE_BELOW_COST"
        print("PASS_A1_TERMINAL_REPAIR_SWARM_V2_SELF_TEST"); return 0
    r=run(args.output)
    print(canonical({"terminal_count":r["terminal_count"],"queued_repair_count":r["queued_repair_count"],"queued_new_arch_count":r["queued_new_arch_count"],"eligible_count":r["eligible_count"],"top":[{"id":x["candidate_id"],"strategy":x["strategy_id"],"mode":x["mode"],"score":x["score"],"eligible":x["eligible_for_preregistration"]} for x in r["global_queue"][:5]]}))
    return 0
