...
def _history_readiness()->dict[str,Any]:
    ...
    try:
        ...
        ratio=float(cov.get("minimum_coverage_progress_ratio") or 0.0)
        ...
        for s in ("basis","funding","open_interest"):
            out[s]={"ready":False,"reason":"P3_COVERAGE_FETCH_FAILED","error":err}
    return out
...
def _source_contract(readiness:Mapping[str,Any])->str:
    allowed=sorted(_allowed_sources(readiness))
    blocked=sorted(k for k in readiness if k not in allowed)
    return (
        ...
        "Do not emit candidates requiring BLOCKED_SOURCES="+canonical(blocked)+". "
        "Native ownership without sufficient history is NOT source-ready. Prefer an economically distinct OHLCV/volume architecture rather than naming unavailable order-flow/carry sources. "
    )

def _batch_prompt(fps:list[dict[str,Any]], evidence:list[dict[str,Any]], readiness:Mapping[str,Any])->str:
    return (
      ...
      +_source_contract(readiness)+
      "Every candidate must include a deterministic executable_spec. Cite supplied evidence_ids, including verified YouTube only as hypothesis evidence and papers as mechanism evidence. Never invent evidence. "
...
def _single_repair_prompt(fail:Mapping[str,Any], evidence:list[dict[str,Any]], readiness:Mapping[str,Any])->str:
    return (
        "You are an economic repair engineer. This candidate already FAILED deterministic development replay. "
        "You get exactly ONE causal repair attempt; no parameter sweep, no threshold tuning, no multiple variants. "
        "Do not merely delete trades. Preserve the payer/mechanism and alter exactly one causal axis that can plausibly exceed REQUIRED_NET_IMPROVEMENT_BPS_PER_TRADE. "
        "If the required improvement is not mechanistically plausible, emit zero candidates. "
        +_source_contract(readiness)+
        "Return at most ONE REPAIR candidate with deterministic executable_spec and evidence_ids. The candidate must use the same development-only authority and 14bps cost. "
...
def run(output:Path)->dict[str,Any]:
    ...
    if _gemini_enabled() and fps:
        try:
            model,raw,lineage=call_gemini_generator(_batch_prompt(fps,source,readiness)); rows=_attach(raw,validate_candidates(raw,"gemini",source_ids,{sid for sid,_ in terminals}),allowed); generated+=rows
    ...
    failure_econ=analyze_failure_economics(dev,queue,SINGLE_REPAIR_BUDGET)

    causal_repairs:list[dict[str,Any]]=[]
    causal_repair_calls=[]
    for fail in failure_econ.get("selected_for_single_repair") or []:
        sid=str(fail.get("strategy_id") or "")
        call_state={"parent_candidate_id":fail.get("candidate_id"),"strategy_id":sid,"required_net_improvement_bps_per_trade":fail.get("required_net_improvement_bps_per_trade")}
        try:
            model,raw,lineage=call_openai_generator(_single_repair_prompt(fail,source,readiness))
            rows=_attach(raw,validate_candidates(raw,"openai",source_ids,{sid}),allowed)
            rows=rows[:1]
            for x in rows:
                x["parent_candidate_id"]=fail.get("candidate_id"); x["repair_iteration"]=1; x["repair_axis_budget_exhausted_after_this"]=True
            causal_repairs.extend(rows)
            call_state.update({"successful":bool(rows),"model":model,"candidate_count":len(rows),**lineage})
        except Exception as exc:
            call_state.update({"successful":False,"candidate_count":0,"error":safe_error(exc)})
    ...
    for x in queue:
        ...
        x["economic_next"]="POST_ECONOMICS_CRITIC" if x["development_economic_pass"] else "REJECT_OR_SINGLE_REPAIR_ONLY_IF_SELECTED"
    repair_pass_ids={str(x.get("candidate_id")) for x in causal_repair_dev.get("passes",[])}
    for x in causal_repairs:
        x["development_economic_pass"]=str(x.get("candidate_id")) in repair_pass_ids
        x["economic_next"]="POST_ECONOMICS_CRITIC" if x["development_economic_pass"] else "TERMINAL_REPAIR_FAIL_NO_MORE_AI"
...
def main()->int:
    ...
    r=run(a.output); print(canonical({"done":r["ledger_done_count"],"terminal":r["terminal_count"],"ready_sources":r["replay_ready_sources"],"machine":r["machine_replayable_count"],"dev_pass":r["development_economic_pass_count"],"failure_economics":r["failure_economics"],"causal_repair_dev":r["causal_repair_development_economics"],"gemini_batch":r["gemini_batch"],"evidence":r["evidence_summary"]})); return 0
