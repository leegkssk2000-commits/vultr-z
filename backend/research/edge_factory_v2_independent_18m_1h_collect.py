...
VERSION='ZEL_EDGE_FACTORY_V2_INDEPENDENT_18M_1H_COLLECT_V1'
SCHEMA='zel.edge_factory_v2.independent_18m_1h_collect.v1'
...
END_EXCLUSIVE_UTC='2024-07-01T00:00:00+00:00'
CSV_FIELDS=('timestamp_ms','open','high','low','close','volume')
PARTITIONS={
 'R1_REPLICATION_DISCOVERY':('2023-01-01T00:00:00+00:00','2023-07-01T00:00:00+00:00'),
 'R2_REPLICATION_VALIDATION':('2023-07-01T00:00:00+00:00','2024-01-01T00:00:00+00:00'),
 'R3_REPLICATION_TEST':('2024-01-01T00:00:00+00:00','2024-07-01T00:00:00+00:00'),
}
...
def normalize_row(raw:Any)->dict[str,Any]:
    if isinstance(raw,Mapping):
        vals={'timestamp_ms':raw.get('openTime',raw.get('time',raw.get('timestamp'))),'open':raw.get('open'),'high':raw.get('high'),'low':raw.get('low'),'close':raw.get('close'),'volume':raw.get('volume',raw.get('vol',0))}
    ...
    while cursor<end_ms:
        ...
        for row in rows:
            ...
            if cursor<=ts<chunk_end:
                ...
                rows_by_ts[ts]=row
        cursor=chunk_end;time.sleep(.08)
    expected=list(expected_timestamps(start_ms,end_ms));missing=[ts for ts in expected if ts not in rows_by_ts];unexpected=sorted(ts for ts in rows_by_ts if ts<start_ms or ts>=end_ms)
...
def main()->int:
    ...
    start,end=utc_ms(START_UTC),utc_ms(END_EXCLUSIVE_UTC);expected_per=(end-start)//INTERVAL_MS
    if expected_per!=13128:raise RuntimeError(f'EXPECTED_ROW_CONTRACT:{expected_per}')
    a.out_dir.mkdir(parents=True,exist_ok=True);results=[collect_symbol(s,a.out_dir,start,end) for s in SYMBOLS]
    for row in results:
        if row['partition_row_counts']!=EXPECTED_PARTS:raise RuntimeError(f"PARTITION_COUNTS:{row['symbol']}:{row['partition_row_counts']}")
    manifest={'schema_version':SCHEMA,'version':VERSION,'generated_at':datetime.now(timezone.utc).isoformat(),'state':'PASS_INDEPENDENT_18M_1H_DATA_STAGED','source':{'base_url':BASE_URL,'endpoint':ENDPOINT,'auth_required':False,'interval':INTERVAL,'chunk_limit':CHUNK_LIMIT,'safe_chunk_bars':SAFE_CHUNK_BARS},'frozen_boundaries':{'start_utc':START_UTC,'end_exclusive_utc':END_EXCLUSIVE_UTC,'start_ms':start,'end_exclusive_ms':end},'partitions':{k:{'start_ms':utc_ms(v[0]),'end_exclusive_ms':utc_ms(v[1]),'rows_per_symbol':EXPECTED_PARTS[k]} for k,v in PARTITIONS.items()},'symbols':list(SYMBOLS),'expected_rows_per_symbol':expected_per,'expected_total_rows':expected_per*len(SYMBOLS),'actual_total_rows':sum(int(r['row_count']) for r in results),'results':results,'economics_inspected':False,'holdout_metrics_inspected':False,'strategy_rules_mutated':False,'ai_used':False,'canonical_mutated':False,'registry_mutated':False,'runtime_mutated':False,'formal_ledger_mutated':False,'shadow_mutated':False,'paper_mutated':False,'live_mutated':False,'protected_mutations':0,'selection_authority':False,'promotion_authority':False,'survivor_declared':False,'execution_authority':'NONE','order_authority':'BLOCKED','action':'hold','next':'REPLICATE_EXACT_FIVE_FROZEN_PRIMITIVES_ON_R1_ONLY'}
    if manifest['actual_total_rows']!=manifest['expected_total_rows']:raise RuntimeError('TOTAL_ROW_COUNT_MISMATCH')
    manifest['dataset_sha256']=stable_sha([{'symbol':r['symbol'],'file_sha256':r['file_sha256']} for r in results]);manifest['receipt_sha256']=stable_sha(manifest);a.manifest.parent.mkdir(parents=True,exist_ok=True);a.manifest.write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n');print(json.dumps({'state':manifest['state'],'actual_total_rows':manifest['actual_total_rows'],'dataset_sha256':manifest['dataset_sha256'],'receipt_sha256':manifest['receipt_sha256']},sort_keys=True));return 0
