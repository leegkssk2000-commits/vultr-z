...
def get_json(path: str, params: dict[str, Any]) -> tuple[Any, str, float]:
    ...
    for base in BASES:
        try:
            ...
            req = urllib.request.Request(url, headers={"User-Agent": "ZEL-P3-readonly/1.0"})
...
def main() -> int:
    ...
    def historical_ok(name: str) -> bool:
        ...
        return len(xs) == 2 and all(p.get("historical_window", {}).get("historical_coverage_for_W1_W2") is True for p in xs)
    ...
    receipt = {
        ...
        "blockers": blockers,
        "interpretation": "Passing current endpoint reads do not substitute for historical aligned coverage. No unobserved endpoint path or threshold is invented.",
