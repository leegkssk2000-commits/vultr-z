...
SCHEMA = "zel.a1.break_and_continue.loss_preentry_attribution.v1"
AUTH = {
...
def run(out: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="break_and_continue_loss_preentry_") as td:
        receipt = diag._run_receipt("break_and_continue", Path(td) / "break_and_continue.json")
    ...
    numeric = []
    for key in ("box_height_atr", "chase_atr", "atr_pct", "ema_spread_atr"):
        a = _mean(streak, key)
    ...
    strong_categorical = [
        ...
        if float(x["loss_streak_share"]) >= 0.75 and float(x["delta_share"]) >= 0.25
    ]
    roots = strong_numeric + strong_categorical
    ...
    compact = [{k: x.get(k) for k in (
        ...
        "box_height_atr", "chase_atr", "atr_pct", "ema_spread_atr"
    )} for x in streak]
    row = {
...
def self_test() -> int:
    ...
    print("PASS_A1_BREAK_AND_CONTINUE_LOSS_PREENTRY_ATTRIBUTION_V1_SELF_TEST")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("out/a1_break_and_continue_loss_preentry_attribution_latest.json"))
