...
SYMBOLS = ("ADA-USDT", "AVAX-USDT", "SUI-USDT", "LTC-USDT")
START_UTC = "2026-03-18T08:20:00+00:00"
END_EXCLUSIVE_UTC = "2026-09-14T08:20:00+00:00"
...
def request_chunk(
    symbol: str, start_ms: int, end_ms: int, *, attempts: int = 5
) -> list[dict[str, Any]]:
    params = {
        "symbol": symbol,
        "interval": INTERVAL,
    ...
    raise RuntimeError(
        f"BINGX_REQUEST_FAILED:{symbol}:{start_ms}:{end_ms}:{last_error}"
    )
...
def collect_symbol(
    symbol: str, out_dir: Path, start_ms: int, end_exclusive_ms: int
) -> dict[str, Any]:
    ...
    while cursor < end_exclusive_ms:
        ...
        chunk_end_exclusive = min(
            cursor + SAFE_CHUNK_BARS * INTERVAL_MS, end_exclusive_ms
        )
    ...
    missing = [
        timestamp for timestamp in expected if timestamp not in rows_by_timestamp
    ]
    unexpected = sorted(
        timestamp
    ...
    output = (
        out_dir / f"{symbol.replace('-', '')}_1m_{start_ms}_{end_exclusive_ms}.csv.gz"
    )
    ...
    return {
        ...
        "end_exclusive_utc": datetime.fromtimestamp(
            end_exclusive_ms / 1000, tz=timezone.utc
        ).isoformat(),
...
def main() -> int:
    ...
    if args.self_test:
        ...
        assert (
            list(expected_timestamps(0, SAFE_CHUNK_BARS * INTERVAL_MS))[-1]
            == (SAFE_CHUNK_BARS - 1) * INTERVAL_MS
        )
        print("PASS")
        return 0
    ...
    results = [
        collect_symbol(symbol, args.out_dir, start_ms, end_exclusive_ms)
    ...
    manifest["dataset_sha256"] = stable_sha(
        [
            {"symbol": row["symbol"], "file_sha256": row["file_sha256"]}
            for row in results
        ]
    )
