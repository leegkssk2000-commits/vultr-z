...
MIN_TRADES = 25
SCHEMA = "zel.a1.keltner.volatility_cool_loss_repair.forward.v2"
...
def run(out: Path) -> dict[str, Any]:
    ...
    base, fastpath = run_terminal_shadow(
        ...
        out=out.parent / "_keltner_fresh_shadow.json",
    )
    ...
    row.update({
        ...
        "state": "WAIT_FRESH_25" if len(trades) < MIN_TRADES else "FRESH_25_READY_FOR_EXISTING_H4_H5",
        "next": "CONTINUE_HOURLY_FRESH_COLLECTION" if len(trades) < MIN_TRADES else "RUN_EXISTING_H4_H5_WITHOUT_RETUNING",
    })
    ...
    shadow = out.parent / "_keltner_fresh_shadow.json"
    if shadow.exists():
        shadow.unlink()
    return row
...
def self_test() -> int:
    ...
    print("PASS_A1_KELTNER_VOLATILITY_COOL_LOSS_REPAIR_FORWARD_V2_SELF_TEST")
    return 0
