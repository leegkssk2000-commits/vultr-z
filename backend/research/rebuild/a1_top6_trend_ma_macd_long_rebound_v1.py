...
SCHEMA='zel.a1.top6.trend_ma_macd.long_rebound.v1'
...
def run(parent_path:Path,symbols:list[str],out:Path)->dict[str,Any]:
    ...
    bars_by,maps,snaps=ab.load_shared_inputs(symbols,authority)
    native=ab.simulate(variant='BASELINE',symbols=symbols,boundary_ms=ab.parse_boundary(boundary),bars_by=bars_by,snapshots=snaps)
    child=[dict(x) for x in native if str(x.get('side'))=='long']
    ...
    r={'schema_version':SCHEMA,'state':'PASS_TOP6_STRUCTURAL_REBOUND_CANDIDATE' if passed else 'ROUTE_TREND_MA_MACD_TO_C_MATERIAL','strategy_id':'trend_ma_macd','axis':'LONG_ONLY_ENTRY_SIDE_QUALIFIER','boundary_utc':boundary,'native':{'metrics':bm,'concentration':bh},'candidate':{'metrics':cm,'concentration':ch,'checks':checks,'trade_retention_pct':100*len(child)/max(1,len(native))},'payoff_is_not_optimization_target_here':True,'fresh_validation_required':True,'production_mutated':False,'selection_authority':False,'promotion_authority':False,'execution_authority':'NONE','order_authority':'BLOCKED','live_trade_authority':'BLOCKED','action':'hold'}
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument('--parent',type=Path); ap.add_argument('--out',type=Path,default=Path('out/a1_top6_trend_ma_macd_long_rebound_v1.json')); ap.add_argument('--symbols',default='BTC-USDT,ETH-USDT,SOL-USDT,XRP-USDT,1INCH-USDT,ETHFI-USDT,HYPE-USDT,BCH-USDT,APE-USDT,1000PEPE-USDT,DOGE-USDT,LINK-USDT'); ap.add_argument('--self-test',action='store_true'); a=ap.parse_args()
    if a.self_test: print('PASS_A1_TOP6_TREND_MA_MACD_LONG_REBOUND_V1_SELF_TEST'); return 0
    if a.parent is None: raise SystemExit('--parent required')
    r=run(a.parent,[x.strip() for x in a.symbols.split(',') if x.strip()],a.out); print(json.dumps({'state':r['state'],'native':r['native']['metrics'],'candidate':r['candidate']['metrics'],'checks':r['candidate']['checks']},sort_keys=True)); return 0
