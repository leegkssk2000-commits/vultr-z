...
from backend.research.rebuild import trend_rider_atr_expansion_child_policy_v1 as child_policy

ROOT = Path(__file__).resolve().parents[3]
INVENTORY = ROOT / "backend/research/rebuild/strategy25_structural_inventory_v2.json"
CHILD_POLICY = ROOT / "backend/research/rebuild/trend_rider_atr_expansion_child_policy_v1.py"
SCHEMA = "zel.a1_trend_rider_atr_expansion_frozen_w123_ab.v1"
...
def _run_exact(out: Path, *, child: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="trend_rider_atr_expansion_ab_") as td:
        ...
        for field in ("entry", "exit", "entry_ts", "exit_ts", "side", "reason", "net_bps", "gross_bps", "realized_cost_bps"):
            ...
            if isinstance(a, (int, float)) and isinstance(b, (int, float)):
                if not math.isclose(float(a), float(b), rel_tol=0.0, abs_tol=1e-9):
                    mismatch.append(f"{key}:{field}:{a}!={b}")
            elif a != b:
                mismatch.append(f"{key}:{field}:{a}!={b}")
        retained.append(copy.deepcopy(crow))
    if mismatch:
        raise RuntimeError("CHILD_PARENT_GEOMETRY_DRIFT:" + "|".join(mismatch[:8]))
...
def run(output: Path) -> dict[str, Any]:
    ...
    child_path = output.parent / "trend_rider_atr_expansion_child_current_receipt.json"
    ...
    direct_integrity = {
        ...
        "child_policy_is_atr_expansion_only": str(child_current.get("policy_path") or "") == str(CHILD_POLICY.relative_to(ROOT)),
    ...
    direct_ab = bool(
        parent_anchor_match
        and all(bool(v) for k, v in direct_integrity.items()
                if k not in {"frozen_observation_run_id", "numeric_threshold_sweep", "best_horizon_selection", "post_outcome_trade_deletion", "uses_post_outcome_data"})
    ...
    deltas = {
        ...
        "trade_retention_pct": {"parent": 100.0, "child": retention,
                                "delta_child_minus_parent": None if retention is None else retention - 100.0,
                                "improvement": None, "lower_is_better": False},
    }
    expectancy_improved = metric_helper._gt(c, p, "net_expectancy_bps")
    robustness_improved = bool(metric_helper._gt(c, p, "profit_factor") or metric_helper._gt(c, p, "payoff") or metric_helper._lt(c, p, "drawdown_bps"))
    ...
    if not parent_anchor_match:
        ...
        state = "HOLD_ATR_EXPANSION_FROZEN_W123_DIRECT_AB_INTEGRITY"
    elif screen_pass:
        state = "PASS_ATR_EXPANSION_FROZEN_W123_DEVELOPMENT_SCREEN_PENDING_H4_H5"
    elif not retention_gate:
        state = "FAIL_ATR_EXPANSION_FROZEN_W123_RETENTION_BELOW_60PCT"
    else:
        state = "FAIL_ATR_EXPANSION_FROZEN_W123_DEVELOPMENT_SCREEN"
    ...
    result = {
        ...
        "changed_axis": AXIS,
        "frozen_observation_authority": {"run_id": FROZEN_OBSERVATION_RUN_ID, "boundary_utc": FROZEN_BOUNDARY_UTC,
                                         "last_post_boundary_ts": FROZEN_LAST_POST_BOUNDARY_TS, "expected_parent": EXPECTED_PARENT},
        ...
        "next_if_fail": "PREENTRY_CONTEXT_AXIS_RESELECTION_REQUIRED",
...
def self_test() -> int:
    ...
    print("PASS_A1_TREND_RIDER_ATR_EXPANSION_FROZEN_W123_AB_V1_SELF_TEST")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("out/a1_trend_rider_atr_expansion_frozen_w123_ab_v1.json"))
    ap.add_argument("--self-test", action="store_true")
    args = ap.parse_args()
    if args.self_test:
        return self_test()
    r = run(args.out)
    print(json.dumps({"state": r["state"], "parent": r["parent_w123"]["aggregate"],
                      "child": r["child_w123"]["aggregate"], "deltas": r["metric_deltas"],
                      "retention_pct": r["trade_retention_pct"], "screen_pass": r["development_screen"]["pass"],
                      "next_if_fail": r["next_if_fail"], "receipt": r["receipt_sha256"]}, sort_keys=True))
    return 0
