...
from backend.research.rebuild import trend_rider_multiscale_alignment_child_policy_v1 as child_policy

ROOT = Path(__file__).resolve().parents[3]
INVENTORY = ROOT / "backend/research/rebuild/strategy25_structural_inventory_v2.json"
CHILD_POLICY = ROOT / "backend/research/rebuild/trend_rider_multiscale_alignment_child_policy_v1.py"
SCHEMA = "zel.a1_trend_rider_multiscale_alignment_frozen_w123_ab.v1"
...
def _run_exact(out: Path, *, child: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="trend_rider_multiscale_alignment_ab_") as td:
...
def run(output: Path) -> dict[str, Any]:
    ...
    child_path = output.parent / "trend_rider_multiscale_alignment_child_current_receipt.json"
    ...
    direct_integrity = {
        ...
        "child_policy_is_multiscale_alignment_only": str(child_current.get("policy_path") or "") == str(CHILD_POLICY.relative_to(ROOT)),
    ...
    direct_ab = bool(
        ...
        and all(
            bool(v)
            for k, v in direct_integrity.items()
            if k not in {"frozen_observation_run_id", "numeric_threshold_sweep", "best_horizon_selection", "post_outcome_trade_deletion"}
        )
    ...
    retention = (
        float(c.get("trades") or 0) / float(p.get("trades") or 1) * 100.0
        if float(p.get("trades") or 0) > 0
        else None
    )
    deltas = {
    ...
    if not parent_anchor_match:
        ...
        state = "HOLD_MULTISCALE_ALIGNMENT_FROZEN_W123_DIRECT_AB_INTEGRITY"
    elif screen_pass:
        state = "PASS_MULTISCALE_ALIGNMENT_FROZEN_W123_DEVELOPMENT_SCREEN_PENDING_H4_H5"
    elif not retention_gate:
        state = "FAIL_MULTISCALE_ALIGNMENT_FROZEN_W123_RETENTION_BELOW_60PCT"
    else:
        state = "FAIL_MULTISCALE_ALIGNMENT_FROZEN_W123_DEVELOPMENT_SCREEN"
    ...
    result = {
        ...
        "alignment_transform": child_policy.ALIGNMENT_TRANSFORM,
...
def self_test() -> int:
    ...
    print("PASS_A1_TREND_RIDER_MULTISCALE_ALIGNMENT_FROZEN_W123_AB_V1_SELF_TEST")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("out/a1_trend_rider_multiscale_alignment_frozen_w123_ab_v1.json"))
