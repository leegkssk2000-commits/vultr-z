...
POLICY = ROOT / "backend/research/rebuild/trend_rider_wr80_us_chase_cooling_child_policy_v1.py"
PREREG = ROOT / "backend/research/rebuild/a1_trend_rider_wr80_us_chase_cooling_prereg_v1.json"
MIN_TRADES = 25
SCHEMA = "zel.a1.trend_rider.wr80_us_chase_cooling.forward.v1"
...
def run(out: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="trend_rider_wr80_restore_") as td:
        td_path = Path(td)
        ...
        if str(base.get("policy_path") or "") != str(POLICY.relative_to(ROOT)):
            raise RuntimeError("WR80_RESTORE_CHILD_POLICY_MISMATCH")
        if list(base.get("integrity_defects") or []):
            raise RuntimeError("WR80_RESTORE_CHILD_INTEGRITY_DEFECT")
        if int(base.get("leakage_lookahead") or 0) != 0:
            raise RuntimeError("WR80_RESTORE_CHILD_LOOKAHEAD_DEFECT")
        if str(base.get("boundary_utc") or "") != boundary:
            raise RuntimeError("WR80_RESTORE_FRESH_BOUNDARY_MISMATCH")
        ...
        candidate.update({
            ...
            "baseline_identity": prereg["baseline_identity"],
            ...
            "discovery_source_receipt_sha256": prereg["discovery_source_receipt_sha256"],
        })
        ...
        if source_state == "FAIL":
            ...
            if hardening.get("state") == "PASS_HARDENING_EVIDENCE":
                state = "PASS_WR80_RESTORE_FRESH_HARDENING"
                nxt = "COMPARE_WITH_PARENT_AND_NON_US_PARTIAL_SUCCESS_THEN_A2_A3"
            else:
                state = "HOLD_WR80_RESTORE_FRESH_HARDENING"
                nxt = "PRESERVE_PARTIAL_SUCCESS_AND_ROUTE_NEXT_WINNER_RESTORE_AXIS"
...
def self_test() -> int:
    prereg = _read(PREREG)
    assert prereg["changed_axis"] == "NON_US_PLUS_US_CHASE_COOLING_OR_FLAT_ONLY"
    assert prereg["fresh_boundary_utc"] == "2026-08-23T13:00:00Z"
    assert prereg["preboundary_outcomes_counted"] is False
    assert prereg["numeric_threshold_sweep"] is False
    assert prereg["outcome_used_at_runtime"] is False
    print("PASS_A1_TREND_RIDER_WR80_US_CHASE_COOLING_FORWARD_V1_SELF_TEST")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("out/a1_trend_rider_wr80_us_chase_cooling_forward_latest.json"))
