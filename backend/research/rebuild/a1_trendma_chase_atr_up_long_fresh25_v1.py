...
POLICY = ROOT / "backend/research/rebuild/trend_ma_macd_chase_atr_up_long_good_child_policy_v1.py"
PREREG = ROOT / "backend/research/rebuild/a1_trendma_chase_atr_up_long_good_prereg_v1.json"
...
def _validate_prereg() -> dict[str, Any]:
    ...
    if p.get("state") != "PASS_PREREG_FROZEN":
        raise RuntimeError("CHASE_LONG_PREREG_NOT_FROZEN")
    if p.get("candidate_identity") != IDENTITY or p.get("cumulative_parent_identity") != CUMULATIVE_PARENT_IDENTITY:
        raise RuntimeError("CHASE_LONG_PREREG_IDENTITY_MISMATCH")
    if p.get("transport_strategy_id") != STRATEGY_ID:
        raise RuntimeError("CHASE_LONG_PREREG_TRANSPORT_MISMATCH")
    if tuple(p.get("fixed_universe") or []) != LIQUID6:
        raise RuntimeError("CHASE_LONG_PREREG_LIQUID6_MISMATCH")
    if int(p.get("minimum_fresh_trades") or 0) != MIN_TRADES or int(p.get("exact_first_completed_trades") or 0) != MIN_TRADES:
        raise RuntimeError("CHASE_LONG_PREREG_SAMPLE_MISMATCH")
    if int(p.get("changed_axis_count_relative_to_cumulative_parent") or 0) != 1:
        raise RuntimeError("CHASE_LONG_DELTA_AXIS_COUNT_MISMATCH")
    if p.get("preboundary_outcomes_counted") is not False or p.get("numeric_threshold_sweep") is not False:
        raise RuntimeError("CHASE_LONG_PREREG_FRESH_CONTRACT_FAIL")
    if p.get("outcome_used_at_runtime") is not False or p.get("combined_with_primary") is not False:
        raise RuntimeError("CHASE_LONG_PREREG_RUNTIME_OR_COMBINATION_FAIL")
    actual_blob = common._git_blob_sha(POLICY)
    if actual_blob != str(p.get("policy_blob_sha") or ""):
        raise RuntimeError(f"CHASE_LONG_POLICY_BLOB_MISMATCH:{actual_blob}:{p.get('policy_blob_sha')}")
    return p
...
def _base(prereg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "zel.a1.trendma_chase_atr_up_long.fresh25.v1",
        "candidate_identity": IDENTITY,
        "cumulative_parent_identity": CUMULATIVE_PARENT_IDENTITY,
        "transport_strategy_id": STRATEGY_ID,
        "strategy_id": STRATEGY_ID,
        "alternative_to_primary": "trend_ma_macd_ema_fast_up_good_v1",
        ...
        "changed_axis_relative_to_cumulative_parent": str(prereg["changed_axis_relative_to_cumulative_parent"]),
        "changed_axis_count_relative_to_cumulative_parent": 1,
        ...
        "cumulative_parent_preserved": True,
        "primary_ema_fast_child_preserved": True,
        "parent_h4_h5_reuse_for_promotion": False,
        "identity_specific_hardening_required_after_25": True,
        "indicator_removal_control": "RESTORE_FROZEN_CHASE_ATR_UP_CUMULATIVE_PARENT_SAME_BOUNDARY",
...
def run(now: datetime | None = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="trendma_chase_atr_up_long_fresh25_") as td:
        td_path = Path(td)
        ...
        if str(raw.get("policy_path") or "") != str(POLICY.relative_to(ROOT)):
            raise RuntimeError(f"CHASE_LONG_POLICY_PATH_MISMATCH:{raw.get('policy_path')}")
        ...
        if source_state == "FAIL":
            ...
            nxt = "PRESERVE_CUMULATIVE_PARENT_AND_CHILD_EVIDENCE_DO_NOT_HARDEN_OR_PROMOTE"
        else:
            result["state"] = "READY_IDENTITY_HARDENING"
            result["next"] = "RUN_MARGINAL_IDENTITY_SPECIFIC_H4_H5_VS_CHASE_PARENT"
            ...
            hardened = nested_hardening.run(candidate_path, hard_path)
            ...
            if hardened.get("state") == "PASS_GOOD_REGIME_IDENTITY_HARDENING":
                state = "PASS_TRENDMA_CHASE_ATR_UP_LONG_FRESH_SURVIVOR_GATE"
                nxt = "COMPARE_CUMULATIVE_CHILD_VS_CHASE_PARENT_AND_EMA_FAST_PRIMARY_NO_PROMOTION_AUTHORITY"
            else:
                state = "HOLD_TRENDMA_CHASE_ATR_UP_LONG_FRESH_HARDENING"
                nxt = "PRESERVE_CUMULATIVE_PARENT_AND_CHILD_EVIDENCE_NO_PROMOTION"
...
def self_test() -> int:
    ...
    assert p["cumulative_parent_identity"] == CUMULATIVE_PARENT_IDENTITY
    assert p["combined_with_primary"] is False
    fake_before = datetime(2026, 8, 23, 17, 50, tzinfo=timezone.utc)
    ...
    assert r["changed_axis_count_relative_to_cumulative_parent"] == 1
    assert r["execution_authority"] == "NONE" and r["order_authority"] == "BLOCKED"
    print("PASS_A1_TRENDMA_CHASE_ATR_UP_LONG_FRESH25_V1_SELF_TEST")
    return 0


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("out/a1_trendma_chase_atr_up_long_fresh25_latest.json"))
