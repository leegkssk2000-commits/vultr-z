...
from backend.research.rebuild import trend_rider_wr80_us_chase_cooling_child_policy_v1 as gate_policy
...
SCHEMA = "zel.a1.trendrider.broad30_wr80_state_addonly.v1"
EPS = 1e-12
...
def compact(row: Mapping[str, Any]) -> dict[str, Any]:
    return {k: row.get(k) for k in (
        ...
        "session", "prior_chase_atr", "chase_atr", "chase_state", "wr80_state_allowed",
    )}
...
def enrich(receipt: Mapping[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    cfg = gate_policy.TrendRiderWR80USChaseCoolingConfig()
    bars_by: dict[str, list[dict[str, Any]]] = {}
    index_by: dict[str, dict[int, int]] = {}
    for symbol in sorted({str(x["symbol"]) for x in rows}):
        bars = [dict(x) for x in ev.fetch_bars(symbol, interval, 1000)]
        bars_by[symbol] = bars
        index_by[symbol] = {int(x["ts_ms"]): i for i, x in enumerate(bars)}
    for row in rows:
        symbol = str(row["symbol"])
        signal_ts = int(row["signal_ts"])
        idx = index_by[symbol].get(signal_ts)
        if idx is None or idx < 64:
            row["feature_missing"] = True
            continue
        f = gate_policy.compute_trend_rider_feature(
            bars_by[symbol][:idx + 1], symbol=symbol, now_ts_ms=signal_ts, config=cfg
        )
        v = dict(f.values)
        row.update({
            "session": str(v.get("session")),
            "prior_chase_atr": v.get("prior_chase_atr"),
            "chase_atr": v.get("chase_atr"),
            "chase_state": str(v.get("chase_state")),
            "wr80_state_allowed": bool(v.get("us_conditional_reenable_allowed")),
...
def run(broad_path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise RuntimeError(f"WR80_STATE_FEATURE_MISSING:{len(missing)}")
    ...
    selected = [dict(x) for x in broad if bool(x["wr80_state_allowed"])]
    ...
    partial_distinct = [
        ...
        if int(x.get("signal_ts") or 0) > broad_boundary and trade_key(x) not in bkeys and bool(x["wr80_state_allowed"])
    ]
    ...
    if not profile_pass:
        state = "REJECT_WR80_STATE_ADDONLY_HISTORICAL_PROFILE_BELOW_BROAD30"
        next_step = "ROTATE_TO_DISTINCT_ENTRY_QUALITY_AXIS"
    elif postlock and p_ok:
        state = "HOLD_WR80_STATE_POSTLOCK_PARTIAL_SOURCE_STRICT_NEEDS_BASE_BROAD_COVERAGE"
        next_step = "BUILD_BASE_BROAD_POSTLOCK_CANDIDATE_PRODUCER_THEN_RECONFIRM"
    elif quarantined and q_ok:
        state = "HOLD_WR80_STATE_PROFILE_PASS_PARTIAL_VALIDATION_PASS_WAIT_COMPLETE_SOURCE"
        next_step = "BUILD_BASE_BROAD_POSTLOCK_CANDIDATE_PRODUCER; DO_NOT_UNION_PARTIAL_ROWS"
    else:
        state = "HOLD_WR80_STATE_PROFILE_ONLY_NOT_VALIDATED"
        next_step = "KEEP_PARENT30_AND_ROTATE_OR_BUILD_COMPLETE_BASE_BROAD_SOURCE"
    ...
    return {
        ...
        "mechanism": "NON_US_OR_US_CHASE_COOLING_OR_FLAT_ADD_ONLY",
        "axis": {
            "id": gate_policy.AXIS,
            "session_taxonomy": gate_policy.SESSION_TAXONOMY,
            ...
            "borrowed_from_locked_primary_wr8125_semantics": True,
        },
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("out/a1_trendrider_broad30_wr80_state_addonly_v1.json"))
    args = ap.parse_args()
