...
RULE_ID = 'KELTNER_M2_SIGNAL_LOW_CLOSE_EXIT_DEV_V1'
TRIGGER = 'CONTEXT_SIGNAL_LOW_INVALIDATION_CLOSE'
EXIT = 'CONTEXT_SIGNAL_LOW_INVALIDATION_NEXT_OPEN'
...
DECISION = "FIRST_LOW_BREACH_CONTEXT"


def decide(state, row, index, signal_low, ema50):
    """One completed-bar decision; JSON state survives restart without rearming."""
    if state["status"] != UNCHECKED or float(row["close"]) >= signal_low:
        return deepcopy(state)
    return {"status": ALLOWED if float(row["close"]) < ema50 else SUPPRESSED,
            "index": index, "ts": row["bar_close_ts"],
            "observed_close": row["close"], "ema50": ema50, "signal_low": signal_low}
...
def path(rows, signal, ema20, ema50, end, enabled):
    ...
    for j in range(ei, min(native_exit, len(rows) - 1) + 1):
        ...
        if not ema_hit and state['status'] == UNCHECKED and low_hit:
            ...
            trace.append({'kind': DECISION, 'signal_index': i, **deepcopy(state)})
        active_low = low_hit and state['status'] == ALLOWED
        if not (ema_hit or active_low):
            continue
...
def replay(rows, bundle, *, eval_start_ms, eval_end_ms, enabled=True,
    ...
    if type(enabled) is not bool:
        raise RuntimeError('M2_BOOL_REQUIRED')
    if not enabled:
        if fixed_signal_indices is not None:
            raise RuntimeError('M2_DISABLED_FIXED_UNSUPPORTED')
    ...
    result['audit'].update(rule=RULE_ID, comparison_type='EXIT_CHANGE',
        change_axis='FIRST_LOW_BREACH_ONCE_CLOSE_BELOW_EMA50_NEXT_OPEN',
        reference_released_by_actual_M2_exit=False,
