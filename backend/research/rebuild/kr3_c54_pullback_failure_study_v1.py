"""One C54 successor/two FULLs; freeze and remote runtime claims mandatory."""
...
PRIOR_OUT='research/development_evidence/KR3_C54_EFFORT_PROGRESS_AFTER_PR1226_V1'
SCOPE='KR3_C54_PULLBACK_FAILURE_AFTER_PR1227_V1'
OUT='research/development_evidence/'+SCOPE
KEY='c54_pullback_failure_allocation';PERIODS=previous.old.PERIODS
...
def dependencies():
    ...
    for name in ('kr3_c54_pullback_failure_v1.py','test_kr3_c54_pullback_failure_v1.py','kr3_c54_pullback_failure_study_v1.py'):
        name='backend/research/rebuild/'+name;v[name]=h(ROOT/name)
    for filename in ('DESIGN.md','PREFLIGHT_PLAN.json','PREFLIGHT_EVENTS.json'):
        name=OUT+'/'+filename;v[name]=h(ROOT/name)
    return v

def freeze(inputs):
    previous.verify_spec(inputs);budget=read(ROOT/PRIOR_OUT/'BUDGET.json');old=read(ROOT/PARENT_OUT/'SPEC.json')
    need((budget['cumulative_actual'],budget['cumulative_actual_evaluations'])==(56,92),'LATEST_COUNTS_NOT56_92')
    need(KEY not in budget,'DUPLICATE_SCOPE')
    spec=dict(scope=SCOPE,candidate=c.RULE_ID,candidate_ordinal=57,evaluation_ordinals=[93,94],
        source_files_sha256=dependencies(),prior_budget_sha256=h(ROOT/PRIOR_OUT/'BUDGET.json'),
        ...
        rule='Keep C54 entry and original C51 exits. Fix minimum low of entire pre-entry EMA20 pullback plus recovery signal. Before C51 ever arms, a held completed close below that floor requests next actual open. All original exits and C51 activation have priority. No change to exchange SL/TP, entry/reference, size or cost.',
        ...
        selection='Previous profit-zone pivot rejected NOT_RUN and not loosened/retried. New initial-failure hypothesis declared before applicability scan: entire pre-entry pullback floor. Preflight found21/8 eligible breaches on C54 path,16/7 original losses and5/1 original wins. Counts only, no candidate exit prices, alternative PnL or threshold scan. Rule chosen on reused DEV, not independent.',
...
def verify_spec(inputs=None):
    ...
    need(spec['prior_budget_sha256']==h(ROOT/PRIOR_OUT/'BUDGET.json'),'OLD_BUDGET_DRIFT')
...
def reserve(per):
    ...
    at=dict(scope=SCOPE,candidate=c.RULE_ID,candidate_ordinal=57,ordinal=93+j,period=per,
...
def execute(per,inputs,remote):
    ...
    budget=read(ROOT/OUT/'BUDGET.json');need(budget['cumulative_actual_evaluations']==92+j,'EVALUATION_COUNT')
    if j==0:
        need(budget['cumulative_actual']==56,'CANDIDATE_COUNT');budget['cumulative_actual']=57;budget['new_candidate_runs']+=1
        budget['candidate_trials'].append(dict(ordinal=57,candidate=c.RULE_ID,first_evaluation=93,scope=SCOPE))
    budget['cumulative_actual_evaluations']+=1;budget[KEY]['started']+=1
    budget['trials'].append(dict(at,status='STARTED',actual_experiment_ordinal=93+j));save_budget(budget)
    try:
        ...
        result['change_axis']='PREARM_WHOLE_PULLBACK_FAILURE'
        comparison['comparison_type']='C54_PREARM_EXIT_ONLY_FULL'
...
def summarize():
    verify_spec();periods={};lines=['# C54 pre-arm whole-pullback failure: measured result','',
    ...
    for per in PERIODS:
        ...
        for label,snap in [('C54',ps),('C57',cs)]:
    ...
    put(ROOT/OUT/'SUMMARY.json',dict(status=status,periods=periods,candidates=57,evaluations=94,new_candidates=1,new_full=2,parent_replay=0,formal_credit=0,independent=False))
    lines+=['',status,'','Existing C54/C51/KR3 remain preserved. No post-result threshold change, sizing/exit change or automatic successor.','Gain/harm, removed/new/open states, same-calendar DD, concentration and winner retention are in the three per-period accounting files.','Review/CI/merge closure is recorded separately.','']
