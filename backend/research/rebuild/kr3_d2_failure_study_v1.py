"""Finite D2 study. Reserve+push precedes execution; no retry and no formal credit."""
...
p=c.p;ROOT=p.ROOT
SCOPE='KR3_D2_SUPPRESSED_FAILURE_AFTER_PR1217_V1'
OUT=p.CAMPAIGN+'/D2_FAILURE'
RUNS=('DEV2025','SEEN2026')
KEY='kr3_d2_failure_allocation'
...
def deps():
    out=p.dependencies()
    for name in ('kr3_d2_failure_exit_v1.py','kr3_d2_failure_study_v1.py','step7_kr3_winner_accounting_v1.py','break_channel_q1_metrics_v1.py'):
...
def freeze():
    return p.native.contracts.seal({'scope':SCOPE,'candidate_id':c.CANDIDATE,'rule':c.RULE,'parent':c.ORIGINAL_D,
       'runs':list(RUNS),'max_candidates':1,'max_executions':2,'candidate_ordinal':47,'evaluation_ordinals':[67,68],
       ...
       'selection':'One structural-failure exit chosen from all-D held-path diagnostics on both USED_DEV periods. 12/4 old D loss origins exhibit later below-EMA50 and frozen-low closes after initial suppression; no child PnL computed before freeze. Retrospective discovery, not independent evidence. No thresholds scanned.',
       'development_checks':['separate_absolute_profitability_from_partial_repair','terminal_net_increases_both_periods','all_cost2_increases_both_periods','marked_DD_not_worse_both_periods','direct_D_winner_profit_retained','same_reference_clock','no_relabel_of_FORMAL_HOLD'],
...
def reserve(period,spec):
    ...
    need(budget['cumulative_actual_evaluations']==66+i,'COUNTER_OR_PRIOR_ATTEMPT_CHANGED')
    if i==0:
        need(KEY not in budget and budget['cumulative_actual']==46,'EXISTING_CANDIDATE_OR_SCOPE')
        ...
        budget['cumulative_actual']=47;budget['new_candidate_runs']+=1
        budget['candidate_trials'].append({'ordinal':47,'candidate':c.CANDIDATE,'scope':SCOPE,'first_evaluation':67,'specification_sha256':spec['receipt_sha256']})
    else:
        need(budget['cumulative_actual']==47 and budget[KEY]['used']==1 and budget[KEY]['completed']==1,'PREVIOUS_PERIOD_INCOMPLETE')
    need(not any(t['actual_experiment_ordinal']==67+i for t in budget['trials']),'DUPLICATE_ORDINAL')
    at={'scope':SCOPE,'period':period,'actual_experiment_ordinal':67+i,'candidate_ordinal':47,'specification_sha256':spec['receipt_sha256'],'status':'RESERVED_BEFORE_EXECUTION_PUSH_REQUIRED','github_run_id':os.environ.get('GITHUB_RUN_ID'),'github_run_attempt':os.environ.get('GITHUB_RUN_ATTEMPT'),'started_ns':time.time_ns(),'no_retry':True}
    p.write_new(ROOT/OUT/period/'ATTEMPT.json',p.canonical(at))
    budget['trials'].append(at);budget[KEY]['used']+=1;budget['cumulative_actual_evaluations']=67+i
        ...
        for symbol,rows in sorted(packet['rows_by'].items()):
            ...
            for category in ('trades','open_observations','events','trace'):
                for row in charged[category]:
                    ...
                    row.update(mechanism_origin_id=f'{symbol}:{rows[index]["bar_close_ts"]}',d2_specification_sha256=spec['receipt_sha256'])
...
def execute(period,packet,spec):
    ...
    need(head==os.environ.get('ZEL_D2_RESERVATION_COMMIT'),'RESERVATION_COMMIT_REQUIRED')
...
def saved_check(spec):
    ...
    if found==2:
        need(budget[KEY]['completed']==2 and budget[KEY]['used']==2,'ALLOCATION_INTEGRITY')
        need(budget['cumulative_actual']>=47 and budget['cumulative_actual_evaluations']>=68,'CUMULATIVE_INTEGRITY')
...
def summarize():
    ...
    for period in RUNS:
        ...
        need(d['reference_events']==child['reference_events'] and d['reference_opportunities']==child['reference_opportunities'],'D_REFERENCE_CLOCK_DRIFT')
        ...
        out[period]={'D_to_D2':a.compare(d,child),'KR3_to_D2':a.compare(kr3,child),'repair_cohorts':a.repair_cohorts(kr3,d,child),'checks':checks,'snapshots':{n:a.snapshot(v) for n,v in [('KR3',kr3),('D',d),('D2',child)]}}
    ...
    lines=['# D2 post-suppression structural-failure exit — actual reused DEV','',c.RULE,'','One new candidate47, two actual FULL evaluations67/68. KR3 and D results reused, not rerun.','All figures are equal fixed-notional trade-bps; no account-return claim. Both periods are USED_DEV and all inherited research cost-proxy limitations apply.','', '|Period|Rule|Closed/open|Win%|Payoff|PF|Terminal net|All-cost2|Marked DD|','|---|---|---:|---:|---:|---:|---:|---:|---:|']+table+['','## Prespecified development checks',json.dumps({k:v['checks'] for k,v in out.items()},indent=2),'','G5A HOLD and formal credit0 remain. Code/CI completion is not economic pass. No parameter retry, no new source/OOS/provider calls/orders.','Full original-signal attribution, lost/restored winners, reopened losses, exposure and symbol concentration: ACCOUNTING.json.','D held-path failure cohorts were discovery diagnostics only; no per-symbol or historical outcome rules were used. Remote completion/merge is recorded separately.','']
