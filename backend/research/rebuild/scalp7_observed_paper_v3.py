"""Actual-clock-barrier Scalp7 paper execution. No exchange order interface.
...
PROFILE = "POST_DECISION_OBSERVED_QUOTE_ACTUAL_CLOCK_BARRIER_V3"
...
def _usable(quote: Mapping[str, Any]) -> int:
    """Validate a preserved actual-clock proof; never derive usability by tolerance."""
    proof = quote.get("clock_proof")
    if not isinstance(proof, Mapping):
        raise PaperError("QUOTE_ACTUAL_CLOCK_PROOF_REQUIRED")
    received = _ms(quote["received_at_ms"])
    native = (
        received if quote.get("source_ts_ms") is None else _ms(quote["source_ts_ms"])
    )
    required = (
        "native_ts_ms",
        "received_at_ms",
    ...
    if any(type(proof.get(k)) is not int or proof[k] < 0 for k in required):
        raise PaperError("QUOTE_CLOCK_PROOF_FIELDS")
    if (
        proof.get("schema") != "scalp7.actual_source_clock_barrier.v3"
        or proof.get("state") != "USABLE_AFTER_ACTUAL_CLOCK_BARRIER"
        or proof["native_ts_ms"] != native
        or proof["received_at_ms"] != received
        or proof["max_wait_ms"] != 5000
        or proof["started_at_ms"] < received
        or proof["usable_at_ms"] < max(native, received, proof["started_at_ms"])
        or quote.get("usable_at_ms") != proof["usable_at_ms"]
        or proof.get("native_timestamp_rewritten") is not False
        or proof.get("receipt_timestamp_rewritten") is not False
        or proof.get("clock_offset_or_tolerance_applied") is not False
    ):
        raise PaperError("QUOTE_CLOCK_PROOF_BINDING")
    samples = proof.get("clock_samples")
    if not isinstance(samples, list) or not samples:
        raise PaperError("QUOTE_ACTUAL_CLOCK_SAMPLES_REQUIRED")
    if any(
        not isinstance(x, dict)
        or any(
            type(x.get(k)) is not int or x[k] < 0 for k in ("wall_ms", "monotonic_ns")
        )
        for x in samples
    ):
        raise PaperError("QUOTE_CLOCK_SAMPLE_FIELDS")
    if samples[0] != {
        "wall_ms": proof["started_at_ms"],
        "monotonic_ns": proof["started_monotonic_ns"],
    } or samples[-1] != {
        "wall_ms": proof["usable_at_ms"],
        "monotonic_ns": proof["usable_monotonic_ns"],
    }:
        raise PaperError("QUOTE_CLOCK_SAMPLE_ENDPOINTS")
    if any(
        b["wall_ms"] < a["wall_ms"] or b["monotonic_ns"] < a["monotonic_ns"]
        for a, b in zip(samples, samples[1:])
    ):
        raise PaperError("QUOTE_CLOCK_PROOF_REVERSED")
    elapsed = proof["usable_monotonic_ns"] - proof["started_monotonic_ns"]
    if (
        not 0 <= elapsed <= 5_000_000_000
        or _num(proof.get("waited_ms")) != elapsed / 1_000_000
    ):
        raise PaperError("QUOTE_CLOCK_PROOF_DEADLINE")
    return int(proof["usable_at_ms"])
...
def normalize_quote(
    ...
    received: int,
    *,
    clock_barrier: Callable[[int, int], dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    if source_times and len(set(source_times)) != 1:
        raise PaperError("QUOTE_SOURCE_TIME_INVALID")
    proof = (clock_barrier or await_native_time)(
        source_times[0] if source_times else received, received
    )
    quote = {
        "clock_proof": proof,
        "usable_at_ms": proof["usable_at_ms"],
        "symbol": symbol,
        "bid": bid,
        "ask": ask,
...
def quote_request(
    ...
    clock_path = stem.with_suffix(".clock.json")
    immutable_bytes(
        clock_path,
        json_bytes(
            {
                "body_sha256": quote["body_sha256"],
                "receipt_sha256": sha_file(receipt_path),
                "clock_proof": quote["clock_proof"],
            }
        ),
    )
    quote.update(
        ...
        receipt_sha256=sha_file(receipt_path),
        clock_proof_path=str(clock_path),
        clock_proof_sha256=sha_file(clock_path),
    )
    return quote
...
def empty_state(freeze_sha: str) -> dict[str, Any]:
    return {
        "schema": "scalp7.observed_paper.state.v3",
...
class ObservedPaper:
    ...
    def _quotes(
        ...
        for name, quote in chosen.items():
            ...
            usable = _usable(quote)
            if requested <= after or usable > now or received < requested:
                return None
            if received - requested > int(self.config["max_quote_request_ms"]):
                return None
            if usable - requested > int(self.config["max_quote_request_ms"]):
                return None
            ...
            if source is not None and (
                int(source) > usable
                or usable - int(source) > int(self.config["max_quote_request_ms"])
            ):
                return None
        ...
        if max(stamps) - min(stamps) > int(self.config["max_pair_skew_ms"]):
            return None
        usable_stamps = [_usable(q) for q in chosen.values()]
        if max(usable_stamps) - min(usable_stamps) > int(
            self.config["max_pair_skew_ms"]
        ):
            return None
        return chosen
    ...
    def _enter(
        ...
        entry_ts = now
        if any(_usable(q) > entry_ts for q in quotes.values()):
            raise PaperError("ENTRY_BEFORE_ACTUAL_CLOCK_PROOF")
        ...
        position = {
            ...
            "entry_available_ts_ms": entry_ts,
            "entry_quote_usable_by_leg": {s: _usable(q) for s, q in quotes.items()},
    ...
    def _close(
        ...
        stamp = now
        if any(_usable(q) > stamp for q in quotes.values()):
            raise PaperError("ACTION_BEFORE_ACTUAL_CLOCK_PROOF")
        ...
        row.update(
            {
                ...
                "entry_available_ts_ms": position["entry_available_ts_ms"],
                "exit_quote_usable_by_leg": {s: _usable(q) for s, q in quotes.items()},
...
def read_signal_projection(
    ...
    if not path.exists():
        if (
            int(cursor.get("offset", 0)) > 0
            or cursor.get("last_record_sha256", "0" * 64) != "0" * 64
        ):
            raise PaperError("CONSUMED_SIGNAL_PROJECTION_MISSING_HOLD")
        return [], dict(cursor)
    raw = path.read_bytes()
...
def runtime_config(
    ...
    for item in config["code_pins"]:
        _pin(item)
    clock_path = Path(__file__).with_name("scalp7_source_clock_v3.py").resolve()
    if not any(
        Path(item["path"]).resolve() == clock_path for item in config["code_pins"]
    ):
        raise PaperError("SHARED_ACTUAL_CLOCK_PIN_REQUIRED")
    if not any(
...
def initialize(out: Path, config_path: Path, now_ms: int) -> dict[str, Any]:
    ...
    result = {
        "schema": "scalp7.observed_paper.freeze.v3",
        ...
        "clock_admission": "ACTUAL_WALL_AT_OR_AFTER_PRESERVED_NATIVE_AND_RECEIPT_MAX_5000MS_MONOTONIC_WAIT",
        "action_timestamp": "ACTUAL_FINAL_PROCESSING_CLOCK_AFTER_ALL_LEG_CLOCK_PROOFS_AND_FRAME_REFRESH",
        "consumed_missing_signal_projection": "HOLD_NO_CURSOR_ADVANCE",
        "source_snapshot_change": "EXACT_RETRY_EXCEPTION_ONLY_NO_STATE_OR_CURSOR_ADVANCE",
        "promotion_authority": False,
        "order_authority": "BLOCKED",
        "live_authority": "BLOCKED",
    }
    ...
    *,
    ...
    for source in config["signal_sources"]:
        ...
        for wrapper in new:
            ...
            admission_now = wall_clock_ms() if now_ms is None else now
    ...
    observed = (
        wall_clock_ms()
        if now_ms is None
        else max([now] + [_usable(q) for q in quotes.values()])
    )
    _, observed_bars, refreshed_receipt = forward.build_current_frames(
        forward_config, observed, bind=False
    )
    action_now = wall_clock_ms() if now_ms is None else observed
    if action_now < observed:
        raise PaperError("LOCAL_CLOCK_REVERSED_AFTER_QUOTE_SNAPSHOT")
    machine.step(quotes, frames, action_now, observed_bars)
...
def poll(
    ...
    *,
    ...
    """A changing source snapshot retries without persisting in-memory cursors."""
    forward = importlib.import_module(
        "backend.research.rebuild.scalp7_fresh_forward_v2"
    )
    try:
        return _poll_once(out, config_path, now_ms=now_ms, fetch=fetch)
    except forward.FreshForwardError as exc:
        if str(exc) != "SOURCE_SNAPSHOT_CHANGED_RETRY_WITHOUT_ADVANCING":
            raise
        result = {
            "state": "RETRY_SOURCE_SNAPSHOT_CHANGED_NO_STATE_ADVANCE",
            "reason": str(exc),
            "observed_at_ms": wall_clock_ms() if now_ms is None else now_ms,
            "execution_profile": PROFILE,
            "state_or_signal_cursor_advanced": False,
            "order_authority": "BLOCKED",
            "live_authority": "BLOCKED",
        }
        atomic_json(out / "STATUS.json", result)
