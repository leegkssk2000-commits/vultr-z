"""Dedicated BingX BTC/ETH trade-price raw archive for the Micro15m sleeve.
    ...
    identity.update(
        {
            ...
            "role": "MICRO15M_TRADE_PRICE_RAW_ONLY",
            "l2_subscribed": False,
...
def validate_observation(
    row: dict[str, Any], pending: dict[str, str], acknowledged: set[str]
) -> str | None:
    frame = row["frame"]
...
async def capture_trade_session(
    ...
    while not stop.is_set():
        ...
        if wire is not None:
            ...
            ack = validate_observation(row, pending, acknowledged)
            if ack:
                archive.append(
