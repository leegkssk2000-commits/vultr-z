...
import json
import unittest
from backend.research.rebuild import keltner_m2_context_v1 as m1
...
class M2ContextTests(unittest.TestCase):
    def test_above_and_equal_suppress_permanently_and_restart(self):
        for first_ema in (96.,97.):
            rows=bars(20); breach(rows,2); breach(rows,3,90.)
            b=bundle(rows,[0]); b['ema50'][2]=first_ema
            out=run(rows,b)
            self.assertEqual(out['trades'][0]['exit_index'],12)
            ds=[e for e in out['trace'] if e['kind']==m1.DECISION]
            self.assertEqual(len(ds),1); self.assertEqual(ds[0]['status'],m1.SUPPRESSED)
            state=m1.decide({'status':m1.UNCHECKED},rows[2],2,98.,first_ema)
            self.assertEqual(m1.decide(json.loads(json.dumps(state)),rows[3],3,98.,100.),state)
            prefix=rows[:3]; bp={k:(v[:3] if k!='signals' else v) for k,v in b.items()}
            self.assertEqual(ds,[e for e in run(prefix,bp)['trace'] if e['kind']==m1.DECISION])
            # Later observations may affect the original D exit, never the first decision.
            rows[3].update(close=1.,low=1.); b['ema50'][3]=99.
            self.assertEqual(ds,[e for e in run(rows,b)['trace'] if e['kind']==m1.DECISION])

    def test_suppression_does_not_disable_D(self):
        rows=bars(20); breach(rows,2); b=bundle(rows,[0]); b['ema50'][2]=96.; b['ema20'][4]=99.
        t=run(rows,b)['trades'][0]
        self.assertEqual((t['exit_index'],t['exit_reason']),(5,'EMA20_NOT_ABOVE_EMA50_NEXT_OPEN'))

    def test_D_tie_and_timeout_do_not_consume_context(self):
        for j in (2,12):
            rows=bars(20); breach(rows,j); b=bundle(rows,[0]); b['ema20'][j]=99.
            self.assertFalse(any(e['kind']==m1.DECISION for e in run(rows,b)['trace']))

    def test_suppressed_reference_checkpoint_restart(self):
        rows=bars(30); breach(rows,2); breach(rows,3,90.); b=bundle(rows,[0,3,13,27]); b['ema50'][2]=96.
        ck=m1.parent.causal_clock(rows,b,eval_start_ms=0,eval_end_ms=30*m1.BAR,stop_after_index=5)
        self.assertEqual(run(rows,b),run(rows,b,reference_checkpoint=json.loads(json.dumps(ck))))
