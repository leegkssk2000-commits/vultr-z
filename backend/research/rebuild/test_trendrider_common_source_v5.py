...
from backend.research.rebuild import trendrider_common_source_v5 as source
from backend.research.rebuild import test_trendrider_rest_ws_timestamp_v2 as calibration_fixtures


def write_synthetic_calibration(root, schema="object", close_delta=source.HOUR_MS - 1):
    # The timestamp decoder also supports a current-candle calibration. Source
    # promotion requires a closed candle; generate the same fixtures one hour
    # later so their unaltered native candle is the previous completed hour.
    def closed_wall_ms(clock):
        return calibration_fixtures.T + source.HOUR_MS + 20_000 + int(clock.elapsed * 1000)
    with patch.object(calibration_fixtures.SyntheticClock, "wall_ms", closed_wall_ms):
        return calibration_fixtures.write_synthetic_calibration(root, schema, close_delta)
...
class SourceV5Tests(unittest.TestCase):
    def setUp(self):
    ...
    def response(self, params, inclusive=True, cap=None):
        self.assertTrue((self.output / "ATTEMPT_STARTED_V5.json").exists())
    ...
    def test_attempt_open_xb_handles_concurrent_claim(self):
        ...
        def race(path, raw):
            if path.name == "ATTEMPT_STARTED_V5.json":
    ...
    def test_current_partial_candle_pass_cannot_authorize_source(self):
        # Timestamp semantics can match an active candle; closed-candle source
        # authorization still rejects it without consuming an acquisition.
        self.semantic_path = calibration_fixtures.write_synthetic_calibration(self.base / "partial_witness")
        self.seal_authorization()
        self.assertEqual(calibration_fixtures.mod.validate_semantic_receipt(self.semantic_path)["state"], "PASS")
        with self.assertRaisesRegex(source.SourceIntegrityError, "CANDLE_NOT_CLOSED"):
            self.collect()
        self.assertEqual(self.calls, [])
        self.assertFalse(self.output.exists())

    def test_v1_semantic_pass_cannot_authorize_v5_source(self):
        from backend.research.rebuild.test_trendrider_rest_ws_timestamp_v1 import write_synthetic_calibration as old_fixture
        self.semantic_path = old_fixture(self.base / "old_witness")
        self.seal_authorization()
        with self.assertRaisesRegex(source.SourceIntegrityError, "REST_WS_SEMANTIC_NOT_PASS"):
            self.collect()
        self.assertEqual(self.calls, [])
        self.assertFalse(self.output.exists())

    def test_authorization_is_bound_to_new_scope_and_v5_schema(self):
        original = self.authorization_path.read_bytes()
        for field, value in (("scope_key", "TRENDRIDER_UNIFIED_REST_WS_TIMESTAMP_AFTER_PR1279_V1"),
                             ("schema", "trendrider.common.source.authorization.v4")):
            authority = json.loads(original)
            authority.pop("receipt_sha256")
            authority[field] = value
            self.authorization_path.write_bytes(source.canonical_bytes(source.v1._sealed(authority)))
            self.authorization_sha = source.digest_bytes(self.authorization_path.read_bytes())
            with self.subTest(field=field), self.assertRaisesRegex(source.SourceIntegrityError, "AUTHORIZATION_BINDING_OR_SEAL_MISMATCH"):
                self.collect()
        self.assertEqual(self.calls, [])
        self.assertFalse(self.output.exists())
    ...
    def test_contract_and_semantic_mutation_prevents_next_get(self):
        for path in (self.contract_path, self.semantic_path, self.authorization_path):
            with self.subTest(path=path.name):
                ...
                self.assertFalse((self.output / "DATA_FREEZE_V5.json").exists())
    ...
    def test_http_error_preserves_raw_and_forbids_retry(self):
        ...
        failure = json.loads((self.output / "FAILURE_MANIFEST_V5.json").read_bytes())
