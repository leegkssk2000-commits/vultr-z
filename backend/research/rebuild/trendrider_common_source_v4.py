...
Version 4 preserves the frozen V2 window and backward pagination. A separately
...
SOURCE_SCHEMA = "trendrider.common.source.freeze.v4"
AUTHORIZATION_SCHEMA = "trendrider.common.source.authorization.v4"
...
def authorization_template(contract_sha256: str, semantic_sha256: str) -> dict[str, Any]:
    ...
    return v1._sealed({
        "schema": AUTHORIZATION_SCHEMA, "state": "AUTHORIZED_SOURCE_ONCE",
...
def _calibration_validate(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    from backend.research.rebuild import trendrider_rest_ws_timestamp_v1 as calibration
...
def collect(contract_path: str | Path, semantic_path: str | Path, output_dir: str | Path, *,
    ...
    marker = v1._sealed({
        "schema": "trendrider.common.source.attempt.v4", "started_at_ms": v1._clock_ms(),
    ...
    try:
        write_once(output / "ATTEMPT_STARTED_V4.json", canonical_bytes(marker))
    ...
    try:
        write_once(output / "SOURCE_CONTRACT_V4.json", contract_raw)
        write_once(output / "REST_WS_TIMESTAMP_SEMANTIC_RECEIPT.json", semantic_raw)
        write_once(output / "SOURCE_AUTHORIZATION_V4.json", authorization_raw)
        ...
        for symbol in SYMBOLS:
            ...
            for page_index in range(MAX_PAGES):
                ...
                page_receipt = v1._sealed({
                    "schema": "trendrider.common.source.page.v4", "symbol": symbol,
        ...
        write_once(output / "DATA_FREEZE_V4.json", canonical_bytes(manifest))
        return manifest
    except Exception as exc:
        failure = v1._sealed({
        ...
        write_once(output / "FAILURE_MANIFEST_V4.json", canonical_bytes(failure))
