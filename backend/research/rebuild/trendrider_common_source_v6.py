...
Version 6 preserves the frozen V2 window and backward pagination. A separately
sealed stored observed WS snapshot and adjacent closed REST witness authorizes
its directly observed native timestamp lane. There is no alias fallback, timestamp shift, retry,
...
SOURCE_SCHEMA = "trendrider.common.source.freeze.v6"
AUTHORIZATION_SCHEMA = "trendrider.common.source.authorization.v6"
...
def _calibration_validate(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    from backend.research.rebuild import trendrider_observed_timestamp_v1 as calibration
...
def validate_semantic_receipt(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    """Recompute the exact observed snapshot / closed REST witness offline.

    WS snapshot receipt is within its native T hour; REST witness is closed.
    The interval boundary T+hour is economic clock metadata, never a provider
    native close field. Historical numeric/serialization conventions stay fixed.
    """
    try:
        receipt = _calibration_validate(path, expected_sha256)
    except Exception as exc:
        raise SourceIntegrityError("BLOCKED_OBSERVED_WS_REST_TIMESTAMP_WITNESS:" + str(exc)) from exc
    ...
    if (receipt.get("schema") != "trendrider.observed.ws.rest.timestamp.receipt.v1"
            or receipt.get("scope_key") != SCOPE or receipt.get("state") != "PASS"
            or lane not in expected_transform
            ...
            or receipt.get("open_ts_rule") != "native_T"
            or receipt.get("close_ts_rule") != "open_ts + 1h"
            or receipt.get("provider_native_close_claim") is not False
            or receipt.get("outcome_independent") is not True):
        raise SourceIntegrityError("BLOCKED_OBSERVED_WS_REST_TIMESTAMP_WITNESS:CANONICAL_LANE")
    if lane == "ARRAY_OPEN_CLOSE":
        delta = receipt.get("rest_native_close_delta_ms")
        if type(delta) is not int or delta not in (HOUR_MS - 1, HOUR_MS):
            raise SourceIntegrityError("BLOCKED_OBSERVED_WS_REST_TIMESTAMP_WITNESS:ARRAY_NATIVE_CLOSE")
    return receipt
...
def collect(contract_path: str | Path, semantic_path: str | Path, output_dir: str | Path, *,
    ...
    # Keep legacy source metadata serialization; bars carry open ts_ms only.
    # This value is separate from the explicit economic interval boundary.
    close_delta = HOUR_MS - 1 if lane == "OBJECT_TIME_OPEN" else semantic["rest_native_close_delta_ms"]
    legacy_close_rule = ("OPEN_PLUS_HOUR_MINUS_1_MS" if lane == "OBJECT_TIME_OPEN"
                         else "IDENTITY_NATIVE_ARRAY_CLOSE_MS")
    output.mkdir(parents=True, exist_ok=True)
    if any(output.iterdir()):
        raise SourceIntegrityError("SOURCE_OUTPUT_NOT_EMPTY_NO_RESUME")
    marker = v1._sealed({
        "schema": "trendrider.common.source.attempt.v6", "scope_key": SCOPE, "started_at_ms": v1._clock_ms(),
    ...
    try:
        write_once(output / "ATTEMPT_STARTED_V6.json", canonical_bytes(marker))
    ...
    try:
        write_once(output / "SOURCE_CONTRACT_V6.json", contract_raw)
        write_once(output / "OBSERVED_WS_REST_TIMESTAMP_RECEIPT.json", semantic_raw)
        write_once(output / "SOURCE_AUTHORIZATION_V6.json", authorization_raw)
        ...
        for symbol in SYMBOLS:
            ...
            for page_index in range(MAX_PAGES):
                ...
                page_receipt = v1._sealed({
                    "schema": "trendrider.common.source.page.v6", "symbol": symbol,
        ...
        manifest = v1._sealed({
            ...
            "canonical_close_rule": legacy_close_rule,
            "canonical_close_delta_ms": close_delta,
            "canonical_close_metadata_role": "LEGACY_SOURCE_SERIALIZATION_ONLY",
            "open_ts_rule": semantic["open_ts_rule"],
            "close_ts_rule": semantic["close_ts_rule"],
            "economic_close_delta_ms": HOUR_MS,
        ...
        write_once(output / "DATA_FREEZE_V6.json", canonical_bytes(manifest))
        return manifest
    except Exception as exc:
        failure = v1._sealed({
        ...
        write_once(output / "FAILURE_MANIFEST_V6.json", canonical_bytes(failure))
