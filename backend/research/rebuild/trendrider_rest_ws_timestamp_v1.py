...
PROTOCOL_SCHEMA = "trendrider.rest.ws.timestamp.protocol.v1"
RECEIPT_SCHEMA = "trendrider.rest.ws.timestamp.semantic.receipt.v1"
MAX_BYTES = 1_048_576
...
def decode_ws(raw: bytes, binary: bool) -> dict:
    ...
    if "data" not in payload and payload.get("id") == SESSION_ID and payload.get("code") == 0:
        return {"kind": "ack"}
...
async def run_calibration(output_dir, protocol_path, authority_path, *, connect=None,
    ...
    observations, rest_observations = [], []
    ...
    try:
        ...
        unchanged()
        if not matched:
            errors.append("NO_EXACT_REST_WS_TIMESTAMP_OHLC_WITNESS")
    except Exception as exc:
    ...
    state = "PASS" if witness is not None else "BLOCKED_REST_WS_TIMESTAMP_WITNESS"
    ...
    result = sealed({"schema": RECEIPT_SCHEMA, "scope_key": SCOPE, "state": state,
        ...
        "witness": witness, "errors": errors, "artifact_sha256": artifact_sha})
    write_once(output / "REST_WS_TIMESTAMP_SEMANTIC_RECEIPT.json", canonical_bytes(result))
    return result
