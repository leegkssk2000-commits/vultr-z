...
POLICY_ID = "FVG_LONG_ENTRY_REQUIRES_TREND_LONG_V1"
...
CHILD_MANIFEST: Mapping[str, Any] = MappingProxyType(
    {
        ...
        "condition": "indicators.trend_long is true",
        "failure_action": "hold",
    }
)


def apply_fvg_trend_alignment(result: Mapping[str, Any]) -> dict[str, Any]:
    """Fail closed on repaired FVG long entries that lack explicit trend alignment."""
    ...
    if side == "long" and action == "enter" and indicators.get("trend_long") is not True:
        ...
        output.update(
            {
                ...
                "action": "hold",
                "size": 0.0,
                "why": "fvg_trend_alignment_gate",
                "skill": "none",
                "confidence": 0.0,
                "tags": original_tags + ["trend_alignment_gate", "child_only"],
            }
        )
        output["indicators"] = {
            **dict(indicators),
...
def load_fvg_trend_aligned_strategy(root: str | Path) -> Callable[..., dict[str, Any]]:
    ...
    def strategy(*args: Any, **kwargs: Any) -> dict[str, Any]:
        result = repaired(*args, **kwargs)
        return apply_fvg_trend_alignment(result)

    strategy.__name__ = "fvg_trend_aligned_child_strategy_v1"
