...
from backend.strategy25.indicator_contract_repair_adapter_v2 import REPAIR_SPECS, transformed_source


class RepairedStrategyV2LoadError(RuntimeError):
    pass


def load_repaired_namespace(root: str | Path, strategy_id: str) -> dict[str, Any]:
    source = transformed_source(root, strategy_id)
    module_name = f"backend.strategy25.repaired_v2_{strategy_id}"
...
def load_repaired_strategy(root: str | Path, strategy_id: str) -> Callable[..., Any]:
    ...
    if not callable(strategy):
        raise RepairedStrategyV2LoadError(f"STRATEGY_CALLABLE_MISSING:{strategy_id}")
    return strategy
