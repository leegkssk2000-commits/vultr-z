...
from r7a4d_integrated_supertrend_cost_aware_lock_replay import run_replay

OUTPUT_DIRNAME = "r7a4d_integrated_supertrend_cost_aware_lock_oos_v1"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Single-causal-repair OOS: unchanged entries plus completed-close cost-aware profit lock"
    )
    ...
    for symbol in symbols:
        try:
            ...
            replay = run_replay(
                ...
                replay_fold_id="BINGX_REAL_OOS_FIXED_WINDOW_COST_AWARE_LOCK",
    ...
    baseline_aggregate = (
        baseline_summary.get("aggregate")
        if isinstance(baseline_summary.get("aggregate"), dict)
        else {}
    )
    baseline_net = _finite_float(baseline_aggregate.get("net_return_pct_sum"))
    baseline_pf = _finite_float(baseline_aggregate.get("net_profit_factor"))
    baseline_wr = _finite_float(baseline_aggregate.get("win_rate_pct"))
    baseline_trades = int(baseline_aggregate.get("trade_count", 0) or 0)
    comparison = {
        "baseline_summary": str(baseline_summary_path),
        "baseline_available": bool(baseline_aggregate),
        "baseline_trade_count": baseline_trades or None,
        "trade_count_delta": trades - baseline_trades if baseline_trades else None,
        "baseline_net_return_pct_sum": baseline_net,
        "net_return_pct_sum_delta": (
            aggregate["net_return_pct_sum"] - baseline_net if baseline_net is not None else None
        ),
        "baseline_net_profit_factor": baseline_pf,
        "net_profit_factor_delta": (
            aggregate["net_profit_factor"] - baseline_pf
            if aggregate["net_profit_factor"] is not None and baseline_pf is not None
            else None
        ),
        "baseline_win_rate_pct": baseline_wr,
        "win_rate_pct_delta": (
            aggregate["win_rate_pct"] - baseline_wr
            if aggregate["win_rate_pct"] is not None and baseline_wr is not None
            else None
        ),
    }
    ...
    economic_survivor = bool(
        ...
        and aggregate["net_profit_factor"] is not None
        and aggregate["net_profit_factor"] > 1.0
        and aggregate["positive_symbol_count"] >= 3
    )
    state = (
        "PASS_R7A4D_COST_AWARE_LOCK_ECONOMIC_SURVIVOR"
        if economic_survivor
        else "HOLD_R7A4D_COST_AWARE_LOCK_NOT_YET_SURVIVOR"
    )
    summary = {
        "state": state,
        ...
        "next_stage": (
            "FREEZE_SINGLE_STRATEGY_AND_RUN_SECOND_NONOVERLAPPING_OOS"
            if economic_survivor
            else "STOP_REPAIR_AND_REASSESS_ENTRY_OR_EXIT_CAUSE"
        ),
    }
    ...
    print(f"PROFIT_LOCK_ARMED_TRADES={armed_trades}")
    print(f"PROFIT_LOCK_EXITS={lock_exits}")
    print(f"BASELINE_NET_DELTA={comparison['net_return_pct_sum_delta']}")
    print(f"BASELINE_PF_DELTA={comparison['net_profit_factor_delta']}")
