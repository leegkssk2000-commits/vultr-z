...
VERSION = "R7A4D_STRATEGY11_BNC_PRIOR_BOX_REPAIR_V1"
SAFETY = {
    "research_only": True,
...
def load_patched_strategy(root: Path, expected_source_sha: str) -> tuple[Any, dict[str, Any]]:
    ...
    old = '    box = df.iloc[-cfg.box_bars:]'
    new = '    box = df.iloc[-(cfg.box_bars + 1):-1]'
    if source.count(old) != 1:
        raise RuntimeError(f"SOURCE_REPLACEMENT_COUNT:{source.count(old)}")
    patched = source.replace(old, new, 1)
    module_name = "backend.strategies.break_and_continue_research_prior_box_v1"
    ...
    return strategy, {
        ...
        "semantic_change": "CURRENT_INCLUDED_BOX_TO_PRIOR_BOX",
        "threshold_changed": False,
        "replacement": {"old_sha": sha_text(old), "new_sha": sha_text(new), "count": 1},
    }
...
def trace(
    ...
    names = (
        "up_break", "down_break", "tight_box", "long_breakout_now", "short_breakout_now",
        "long_reclaim", "short_reclaim", "trend_long", "trend_short",
        "long_setup", "short_setup", "long_beam", "short_beam", "late_chase_block",
    )
    ...
    predicates = []
    for name in names:
        observed = true_counts[name] + false_counts[name]
        predicates.append({
            "predicate": name,
            "true_count": true_counts[name],
            "false_count": false_counts[name],
            "observed_count": observed,
            "true_rate_pct": true_counts[name] / max(1, observed) * 100.0,
        })
    return {
        "call_count": calls,
        ...
        "up_break_true_count": true_counts["up_break"],
        "down_break_true_count": true_counts["down_break"],
        "long_breakout_now_true_count": true_counts["long_breakout_now"],
        "short_breakout_now_true_count": true_counts["short_breakout_now"],
        "long_setup_true_count": true_counts["long_setup"],
        "short_setup_true_count": true_counts["short_setup"],
...
def main() -> int:
    ...
    control = replay.evaluate(
        variant_id="CONTROL_CURRENT_INCLUDED_BOX",
        config={**source_config, "candidate_id": "CONTROL_CURRENT_INCLUDED_BOX", "axis": "CONTROL"},
        exit_spec=exit_spec,
        strategy=control_strategy,
        **common,
    )
    candidate = replay.evaluate(
        variant_id="PRIOR_BOX_RANGE",
        config={
            **source_config,
            "candidate_id": "PRIOR_BOX_RANGE",
    ...
    result = {
        "schema_version": "strategy11.bnc_prior_box_repair.v1",
        "version": VERSION,
        "state": "PASS_BNC_PRIOR_BOX_DIAGNOSTIC_COMPLETE",
        ...
        "next": "CANONICAL_MINIMAL_PATCH_REVIEW" if relation["state"] == "PASS_DIAGNOSTIC_REPAIR" else "DECOMPOSE_BNC_WINDOW_SYMBOL_EXIT_OR_WAIT_W1",
        "ai_review_state": "WAIT_GROQ_QUOTA",
    ...
    print(result["state"], before["long_breakout_now_true_count"], after["long_breakout_now_true_count"], candidate.get("trade_count"))
    return 0
