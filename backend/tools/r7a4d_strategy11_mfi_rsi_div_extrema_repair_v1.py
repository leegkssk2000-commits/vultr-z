...
STRATEGY_ID = "mfi_rsi_div"
SOURCE_PATH = Path("backend/strategies/mfi_rsi_div.py")
VERSION = "R7A4D_STRATEGY11_MFI_RSI_DIV_EXTREMA_REPAIR_V1"
SAFETY = {
    "research_only": True,
...
def patch_source(source: str) -> tuple[str, list[dict[str, Any]]]:
    replacements = [
        (
            ...
            '    recent = df.iloc[-(cfg.swing_lookback + 1):-1]',
            'EXCLUDE_CURRENT_BAR_FROM_SWING_EXTREMA',
        ),
        (
            '    price_makes_lower_low = price < swing_low_price * (1.0 - cfg.min_div_buffer_pct)',
            '    price_makes_lower_low = low < swing_low_price * (1.0 - cfg.min_div_buffer_pct)',
            'COMPARE_CURRENT_LOW_TO_PRIOR_SWING_LOW',
        ),
        (
            '    price_makes_higher_high = price > swing_high_price * (1.0 + cfg.min_div_buffer_pct)',
            '    price_makes_higher_high = high > swing_high_price * (1.0 + cfg.min_div_buffer_pct)',
            'COMPARE_CURRENT_HIGH_TO_PRIOR_SWING_HIGH',
        ),
    ]
    patched = source
...
def load_patched_strategy(root: Path, expected_source_sha: str) -> tuple[Any, dict[str, Any]]:
    ...
    module_name = "backend.strategies.mfi_rsi_div_research_prior_extrema_v1"
    ...
    return strategy, {
        ...
        "semantic_change": "CURRENT_INCLUDED_CLOSE_EXTREMA_TO_PRIOR_EXTREMA_CURRENT_WICK",
    }
...
def reason_trace(
    ...
    predicate_true: Counter[str] = Counter()
    predicate_false: Counter[str] = Counter()
    calls = 0
    predicate_names = (
                ...
                if isinstance(indicators, Mapping):
                    for name in predicate_names:
                        ...
                        if isinstance(value, bool):
                            (predicate_true if value else predicate_false)[name] += 1
    predicates = []
    for name in predicate_names:
        true_count = predicate_true[name]
        false_count = predicate_false[name]
    ...
    return {
        ...
        "lower_low_true_count": predicate_true["price_makes_lower_low"],
        "higher_high_true_count": predicate_true["price_makes_higher_high"],
        "long_setup_true_count": predicate_true["long_setup"],
        "short_setup_true_count": predicate_true["short_setup"],
...
def main() -> int:
    ...
    control = replay.evaluate(
        variant_id="CONTROL_CURRENT_INCLUDED_CLOSE_EXTREMA",
        config={**source_config, "candidate_id": "CONTROL_CURRENT_INCLUDED_CLOSE_EXTREMA", "axis": "CONTROL"},
        exit_spec=exit_spec,
    ...
    candidate_config = {
        **source_config,
        "candidate_id": "PRIOR_EXTREMA_CURRENT_WICK",
        "axis": "DIVERGENCE_STRUCTURE_DEFINITION",
        "repair": patch_manifest,
    }
    candidate = replay.evaluate(
        variant_id="PRIOR_EXTREMA_CURRENT_WICK",
    ...
    result = {
        "schema_version": "strategy11.mfi_rsi_div_extrema_repair.v1",
        "version": VERSION,
        "state": "PASS_MFI_RSI_DIV_EXTREMA_DIAGNOSTIC_COMPLETE",
        ...
        "next": "CANONICAL_MINIMAL_PATCH_REVIEW" if relation["state"] == "PASS_DIAGNOSTIC_REPAIR" else "TRACE_NEXT_DIVERGENCE_RECLAIM_OR_GATE_BLOCKER",
        "ai_review_state": "WAIT_GROQ_QUOTA",
    ...
    print(
        result["state"],
        before_trace["lower_low_true_count"],
        after_trace["lower_low_true_count"],
        before_trace["higher_high_true_count"],
        after_trace["higher_high_true_count"],
