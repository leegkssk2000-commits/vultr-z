...
from backend.tools import r7a4d_strategy11_rbreaker_source_causal_trace_v1 as blocker
...
SOURCE_PATH = Path("backend/strategies/rbreaker_like.py")
VERSION = "R7A4D_STRATEGY11_RBREAKER_SIGNAL_BOUNDARY_DISTANCE_V1"
SAFETY = {
    "research_only": True,
...
def load_patched_strategy(root: Path, expected_source_sha: str) -> tuple[Any, dict[str, Any]]:
    ...
    new_distance = '''    if long_break:\n        dist_beyond_signal_atr = max(price - breakout_buy, 0.0) / max(atr_now, 1e-9)\n    elif short_break:\n        dist_beyond_signal_atr = max(breakout_sell - price, 0.0) / max(atr_now, 1e-9)\n    elif long_reversal:\n        dist_beyond_signal_atr = max(price - (mid + atr_now * cfg.reversal_reclaim_atr), 0.0) / max(atr_now, 1e-9)\n    elif short_reversal:\n        dist_beyond_signal_atr = max((mid - atr_now * cfg.reversal_reclaim_atr) - price, 0.0) / max(atr_now, 1e-9)\n    else:\n        dist_beyond_signal_atr = 0.0'''
    replacements = (
        ...
        ('        "dist_from_fast_atr": round(dist_from_fast_atr, 6),', '        "dist_beyond_signal_atr": round(dist_beyond_signal_atr, 6),'),
    )
    patched = source
    manifest_rows = []
    ...
    module_name = "backend.strategies.rbreaker_like_research_signal_boundary_v1"
    ...
    return strategy, {
        ...
        "semantic_change": "EMA_DISTANCE_TO_ACTIVE_SIGNAL_BOUNDARY_OVERSHOOT",
...
def compare(candidate: Mapping[str, Any], control: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "payoff_above_one": metric(candidate.get("payoff_ratio")) > 1.0,
...
def main() -> int:
    ...
    control = replay.evaluate(
        variant_id="CONTROL_EMA_DISTANCE",
        config={**source_config, "candidate_id": "CONTROL_EMA_DISTANCE", "axis": "CONTROL"},
        exit_spec=exit_spec,
        strategy=control_strategy,
        **common,
    )
    candidate = replay.evaluate(
        variant_id="ACTIVE_SIGNAL_BOUNDARY_DISTANCE",
        config={
            **source_config,
            "candidate_id": "ACTIVE_SIGNAL_BOUNDARY_DISTANCE",
    ...
    result = {
        "schema_version": "strategy11.rbreaker_signal_boundary_distance.v1",
        "version": VERSION,
        "state": "PASS_RBREAKER_SIGNAL_BOUNDARY_DIAGNOSTIC_COMPLETE",
        ...
        "next": "CANONICAL_MINIMAL_PATCH_REVIEW" if relation["state"] == "PASS_DIAGNOSTIC_REPAIR" else "WAIT_W1_OR_DECOMPOSE_RBREAKER_ECONOMICS",
        "ai_review_state": "WAIT_GROQ_QUOTA",
