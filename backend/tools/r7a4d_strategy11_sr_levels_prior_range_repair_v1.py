...
SOURCE_PATH = Path("backend/strategies/sr_levels.py")
VERSION = "R7A4D_STRATEGY11_SR_LEVELS_PRIOR_RANGE_REPAIR_V1"
SAFETY = {
    "research_only": True,
...
def load_patched_strategy(root: Path, expected_source_sha: str) -> tuple[Any, dict[str, Any]]:
    ...
    module_name = "backend.strategies.sr_levels_research_prior_range_v1"
    ...
    return strategy, {
        ...
        "semantic_change": "CURRENT_INCLUDED_SR_RANGE_TO_PRIOR_SR_RANGE",
        "replacement": {
...
def trace(
    ...
    return {
        ...
        "long_reclaim_true_count": true_counts["reclaim_long"],
        "short_reclaim_true_count": true_counts["reclaim_short"],
...
def main() -> int:
    ...
    result = {
        "schema_version": "strategy11.sr_levels_prior_range_repair.v1",
        "version": VERSION,
        "state": "PASS_SR_LEVELS_PRIOR_RANGE_DIAGNOSTIC_COMPLETE",
        ...
        "next": "CANONICAL_MINIMAL_PATCH_REVIEW" if relation["state"] == "PASS_DIAGNOSTIC_REPAIR" else "TRACE_SR_BREAKOUT_QUALITY_OR_EXIT_STRUCTURE",
        "ai_review_state": "WAIT_GROQ_QUOTA",
