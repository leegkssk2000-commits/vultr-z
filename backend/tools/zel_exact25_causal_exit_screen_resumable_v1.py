...
VERSION = "ZEL_EXACT25_CAUSAL_EXIT_SCREEN_RESUMABLE_V1"
SCHEMA = "zel.exact25.causal_exit_screen.resumable.receipt.v1"
CHECKPOINT_SCHEMA = "zel.exact25.causal_exit_screen.checkpoint.v1"
HEARTBEAT_SCHEMA = "zel.exact25.causal_exit_screen.heartbeat.v1"
...
def validate_external_receipts(
    source_owner: Mapping[str, Any], route: Mapping[str, Any]
) -> None:
    ...
    if route.get("selected_route") != "EXIT_CAPTURE_INTRATRADE_REPLAY":
        raise RuntimeError("ROUTE_CONFIRMATION_MISMATCH")
...
def baseline_body(
    ...
    card, rows, extra = screen.replay(
        engine, source_root, data_root, strategy_id, None
    )
    immutable_rows = screen.terminal_rows(
        terminal_root / "trades.jsonl.gz", strategy_id
    )
    report = screen.read_json(terminal_root / "report.json")
    terminal_card = next(
        row
    ...
    parity = {
        ...
        "economic_digest": screen.economic_digest(rows)
        ...
        <= 1e-9,
        "errors_zero": int(extra.get("error_count") or 0) == 0,
        "censored_zero": int(extra.get("censored_open_at_window_end") or 0) == 0,
    ...
    return {
        ...
        "baseline": screen.by_window(engine, rows),
        "baseline_integrity": integrity,
        "immutable_integrity": immutable_integrity,
        "baseline_economic_digest_sha256": screen.economic_digest(rows),
        "immutable_economic_digest_sha256": screen.economic_digest(immutable_rows),
    }
...
def candidate_body(
    ...
    strategy_id = str(policy["strategy_id"])
    _card, rows, extra = screen.replay(
        engine, source_root, data_root, strategy_id, config
    )
    windows = screen.by_window(engine, rows)
    gate_policy = policy["positive_gate"]
    w1_ok, w1_blockers, w1_retention = screen.gate_window(
        windows["1m_w1"],
    ...
    integrity = row_integrity(rows)
    operational_ok = (
        int(extra.get("error_count") or 0) <= int(gate_policy["error_count_max"])
        and int(extra.get("censored_open_at_window_end") or 0)
        <= int(gate_policy["censored_open_count_max"])
        and (
            int(extra.get("suppressed_price_exit_count") or 0)
            + int(extra.get("suppressed_strategy_exit_count") or 0)
        )
        > 0
    ...
    if int(extra.get("error_count") or 0) > int(gate_policy["error_count_max"]):
        blockers.append("ERROR_COUNT_ABOVE_MAX")
    if int(extra.get("censored_open_at_window_end") or 0) > int(
    ...
    *,
    ...
    if selected is not None:
        for window in ("1m_w2", "1m_w3"):
            passed, blockers, retention = screen.gate_window(
    ...
    receipt = {
        ...
        "version": VERSION,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "state": "PASS_CAUSAL_EXIT_SCREEN_RESUMABLE_COMPLETE",
        "input_fingerprint": fingerprint,
        ...
        "selection_protocol": {
            ...
            "oracle_mfe_future_value_forbidden": True,
        },
...
def run(
    *,
    ...
    screen = load_module(screen_path, f"zel_causal_screen_{os.getpid()}")
    policy = read_json(policy_path)
    source_owner = read_json(source_owner_path)
    route = read_json(route_path)
    validate_external_receipts(source_owner, route)
    fingerprint = stable_sha(
        {
            ...
            "route_sha256": file_sha(route_path),
    ...
    engine = screen.load_module(
        engine_path, f"zel_causal_exit_engine_resumable_{os.getpid()}"
    )
    try:
        ...
        for ordinal, config in enumerate(policy["candidate_configs"], 1):
            ...
            if path.exists():
                body = validate_checkpoint(
                    path, fingerprint=fingerprint, kind=unit
                )
            else:
