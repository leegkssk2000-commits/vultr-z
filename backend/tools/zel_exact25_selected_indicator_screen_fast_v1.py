...
def window_pass_with_payoff(
    ...
    passed, delta, blockers = screen.window_pass(
        base, candidate, minimum_retention, minimum_count
    )
    delta = dict(delta)
    blockers = list(blockers)
    delta["delta_payoff_ratio"] = float(candidate.get("payoff_ratio") or 0.0) - float(
        base.get("payoff_ratio") or 0.0
    )
    delta["delta_expectancy_R"] = float(candidate.get("expectancy_R") or 0.0) - float(
...
def self_test() -> int:
    ...
    passed, delta, blockers = window_pass_with_payoff(base, candidate, 60.0, 20)
    assert passed is True and not blockers
    assert delta["delta_payoff_ratio"] > 0
    assert delta["delta_expectancy_R"] > 0
    print("PASS")
    return 0
