...
def constants(node: ast.AST | None) -> dict[str, list[Any]]:
    ...
    if node is not None:
        for child in ast.walk(node):
            ...
            elif isinstance(value, (int, float)) and not isinstance(value, bool) and -1_000_000 <= float(value) <= 1_000_000:
                numbers.add(value)
    ...
    for child in ast.walk(node):
        ...
        if child.args and isinstance(child.args[0], ast.Constant) and isinstance(child.args[0].value, str):
            ...
            if SECRET_KEY.search(key):
                key = "<redacted>"
        default = constants(child.args[1]) if len(child.args) > 1 else {"strings": [], "numbers": []}
        rows.append(
            {
...
def fingerprint(node: ast.AST | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "calls": sorted({call_name(child) for child in ast.walk(node) if isinstance(child, ast.Call)})[:160],
...
def target_key(node: ast.AST) -> str | None:
    ...
    if isinstance(node, ast.Subscript):
        value = subscript_key(node)
        return value if value in TARGET_KEYS else None
    return None
        ...
        if isinstance(child, ast.Assign):
            ...
            for key_node, value_node in zip(child.keys, child.values):
                if isinstance(key_node, ast.Constant) and key_node.value in TARGET_KEYS:
                    writers.append({"key": key_node.value, "line": int(getattr(key_node, "lineno", child.lineno)), "kind": "dict", "value": fingerprint(value_node)})
        elif isinstance(child, ast.Call):
            name = call_name(child)
            if (name.endswith(".get") or name == "get") and child.args and isinstance(child.args[0], ast.Constant):
                key = child.args[0].value
                if key in TARGET_KEYS:
                    readers.append({"key": key, "line": int(child.lineno), "kind": "get", "base": name.removesuffix(".get")})
        elif isinstance(child, ast.Subscript):
            key = subscript_key(child)
            if key in TARGET_KEYS and isinstance(child.ctx, ast.Load):
                readers.append({"key": key, "line": int(getattr(child, "lineno", 0) or 0), "kind": "subscript", "base": dotted_name(child.value)})
...
def candidate_paths(root: Path) -> Iterable[Path]:
    ...
    for base, dirs, files in os.walk(root):
        dirs[:] = [name for name in dirs if name not in {".git", ".venv", "venv", "node_modules", "__pycache__", "archive", "archives", "backup", "backups"}]
        for name in files:
        ...
        for path in candidate_paths(root):
            ...
            for function in [node for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))]:
                ...
                record = {
                    ...
                    "writers": function_writers,
                    "readers": function_readers,
                }
                functions.append(record)
                for row in function_writers:
                    writers.append({"root_label": label, "path": key, "sha256": sha256_file(path), **record | {"writers": None, "readers": None}, **row})
                for row in function_readers:
                    readers.append({"root_label": label, "path": key, "sha256": sha256_file(path), **record | {"writers": None, "readers": None}, **row})
            files.append({"root_label": label, "path": key, "sha256": sha256_file(path), "functions": functions, "raw_code_published": False})
