...
VERSION = "ZEL_HISTORICAL_OOS_LANE_CHECKPOINT_V2_SOURCE_HASH_MIGRATION"
_WORKER_ENGINE: Any = None
LANE_SCHEMA = "zel.historical_oos_exact25_replay.lane_checkpoint.v1"
...
def _read_lane_payload(path: Path) -> dict[str, Any] | None:
    try:
...
def load_lane_compatible(path: Path, fingerprint: str, strategy_id: str, row: Mapping[str, Any]) -> tuple[dict[str, Any] | None, bool]:
    """Reuse a lane across manifest-receipt churn only when the actual source file hash matches.

    The old replay fingerprint included manifest.json byte SHA, so semantically identical
    materializations invalidated every checkpoint when receipt/generated metadata changed.
    Lane checkpoints already carry source_sha256, which is the correct immutable data key.
    We therefore accept an old fingerprint only if strategy/window/symbol/interval and
    source_sha256 all match exactly, then atomically rewrite it under the current fingerprint.
    """
    payload = _read_lane_payload(path)
    if payload is None or payload.get("schema_version") != LANE_SCHEMA:
        return None, False
    if payload.get("strategy_id") != strategy_id:
        return None, False
    if payload.get("window_id") != str(row["window_id"]):
        return None, False
    if payload.get("symbol") != str(row["symbol"]):
        return None, False
    if payload.get("interval") != str(row["interval"]):
        return None, False
    ...
    if result.get("window_id") != str(row["window_id"]) or result.get("symbol") != str(row["symbol"]):
        return None, False
    return result, payload.get("input_fingerprint") != fingerprint
...
def self_test() -> None:
    ...
    row = {"window_id": "W1", "symbol": "BTCUSDT", "interval": "1m", "sha256": "data-sha"}
    assert lane_key("trend_rider", row) == "trend_rider__W1__BTCUSDT__1m"
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "lane.json.gz"
        result = {"strategy_id": "trend_rider", "window_id": "W1", "symbol": "BTCUSDT"}
        save_lane(path, "old-fingerprint", "trend_rider", row, result)
        loaded, migrate = load_lane_compatible(path, "new-fingerprint", "trend_rider", row)
        assert loaded == result and migrate is True
        wrong = dict(row); wrong["sha256"] = "different-data"
        loaded, _ = load_lane_compatible(path, "new-fingerprint", "trend_rider", wrong)
    ...
    print(json.dumps({
        ...
        "version": VERSION,
        "source_hash_checkpoint_migration": True,
        "fingerprint_mismatch_alone_does_not_force_recompute": True,
    }, sort_keys=True))
...
def main() -> int:
    ...
    if any(not row.get("sha256") for row in files):
        raise RuntimeError("LANE_SOURCE_SHA256_MISSING")

    strategy_results: dict[str, dict[str, Any]] = {}
    lane_results: dict[tuple[str, str, str], dict[str, Any]] = {}
    missing_strategies: list[str] = []
    migrated_lane_count = 0
    ...
    for strategy_id in sorted(registry):
        ...
        for row in files:
            ...
            if not lpath.exists():
                continue
            loaded, migrate = load_lane_compatible(lpath, fingerprint, strategy_id, row)
            if loaded is None:
                continue
            if migrate:
                save_lane(lpath, fingerprint, strategy_id, row, loaded)
                migrated_lane_count += 1
    ...
    print(json.dumps({
        ...
        "version": VERSION,
        "strategy_checkpoints_current_fingerprint": len(strategy_results),
        "missing_strategies": missing_strategies,
        "compatible_lane_checkpoints": len(lane_results),
        "migrated_lane_checkpoints": migrated_lane_count,
