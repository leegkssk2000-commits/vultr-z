...
SCHEMA = "zel.pipeline_hygiene_audit.v1"
...
def audit(repo_root: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if row["git_push"]:
            for ref in row["latest_json_refs"]:
                latest_writers[ref].append(row["path"])

    duplicate_latest_writers = {
        ref: sorted(set(paths))
        for ref, paths in latest_writers.items()
        if len(set(paths)) > 1
    }
    ...
    if duplicate_latest_writers:
        recommendations.append("ELIMINATE_DUPLICATE_LATEST_JSON_WRITERS_BEFORE_TUNING_MORE_RESEARCH")
    if long_retention:
    ...
    state = "HOLD_PIPELINE_HYGIENE_RISKS_PRESENT" if (
        duplicate_latest_writers or any(x["risk_score"] >= 8 for x in rows)
    ...
    return {
        ...
        "metrics": {
            ...
            "duplicate_latest_json_writer_count": len(duplicate_latest_writers),
        },
        "cron_minute_density": dict(sorted(cron_minutes.items(), key=lambda kv: (-kv[1], kv[0]))),
        "duplicate_latest_json_writers": duplicate_latest_writers,
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zel-pipeline-hygiene-") as td:
        ...
        (wf / "alpha-v1.yml").write_text(
            ...
            "      - run: cp out/x.json backend/x_latest.json\n",
            encoding="utf-8",
        )
        r = audit(root)
        assert r["metrics"]["workflow_count"] == 1, r
        assert r["metrics"]["scheduled_git_push_writer_count"] == 1, r
        assert r["metrics"]["artifact_retention_gt30d_count"] == 1, r
        assert r["metrics"]["workflow_version_mismatch_count"] == 1, r
        assert r["read_only"] is True and r["protected_mutations"] == 0, r
    print("PASS_ZEL_PIPELINE_HYGIENE_AUDIT_V1_SELF_TEST")
    return 0
