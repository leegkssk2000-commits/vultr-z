...
def _validate_nonnegative_ms(name: str, value: Any) -> None:
...
class BotRequest:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_nonnegative_ms("freshness_ms", self.freshness_ms)
        _validate_nonnegative_ms("latency_ms", self.latency_ms)
...
class BotResponse:
    ...
    def __post_init__(self) -> None:
        ...
        missing = sorted(name for name, value in required.items() if not _text(value))
        if missing:
            raise ValueError(f"MISSING_RESPONSE_FIELDS:{','.join(missing)}")
