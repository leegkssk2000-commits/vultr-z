/* ZEL APP V56 - V47 minimal live feed core, UI-only fixed
...
(function () {
  ...
  function cleanOldArtifacts(card) {
    ...
    var kill = [
      ...
      '[data-zel-live-core]','.zel-v55-live-core','.zel-v54-live-card .zel-v54-spark-wrap','.zel-v54-live-core','.zel-v53-live-core','.zel-v52-live-core','.zel-v51-live-core',
    ...
    kill.forEach(function (q) {
      Array.prototype.slice.call(card.querySelectorAll(q)).forEach(function (el) {
        if (!el.closest('.zel-v56-spark-wrap')) el.remove();
      });
    });
  }
  ...
  function findCard(sym) {
    ...
    all.forEach(function (el) {
      if (el.closest('.zel-v56-spark-wrap')) return;
  ...
  function titleCandidate(card, sym) {
    ...
    nodes.forEach(function (el) {
      if (!visible(el) || el.closest('.zel-v56-spark-wrap')) return;
      var t = txt(el);
  ...
  function ensureTitle(card, sym) {
    var title = card.querySelector('[data-zel-v56-price-title="' + sym + '"]') || titleCandidate(card, sym);
    if (!title) {
      var label = symbolLabel(card, sym);
      title = document.createElement('div');
      title.className = 'zel-v56-price-title';
      title.setAttribute('data-zel-v56-price-title', sym);
    ...
    title.setAttribute('data-zel-v56-price-title', sym);
    title.classList.add('zel-v56-price-title');
    return title;
  }

  function ensureStatus(card, sym, title) {
    var st = card.querySelector('[data-zel-v56-feed-status="' + sym + '"]');
    if (!st) {
      st = document.createElement('div');
      st.className = 'zel-v56-feed-status';
      st.setAttribute('data-zel-v56-feed-status', sym);
  ...
  function ensureSpark(card, sym, title, status) {
    var wrap = card.querySelector('.zel-v56-spark-wrap[data-symbol="' + sym + '"]');
    if (!wrap) {
      wrap = document.createElement('div');
      wrap.className = 'zel-v56-spark-wrap';
      wrap.setAttribute('data-symbol', sym);
      var canvas = document.createElement('canvas');
      canvas.className = 'zel-v56-spark-canvas';
  ...
  function hideLegacyStrips(card) {
    ...
    Array.prototype.slice.call(card.querySelectorAll('div,span,i,b')).forEach(function (el) {
      ...
      if (el.hasAttribute('data-zel-v56-price-title') || el.hasAttribute('data-zel-v56-feed-status')) return;
      var t = txt(el);
      ...
      el.setAttribute('data-zel-v56-hidden-strip', '1');
    ...
    Array.prototype.slice.call(card.querySelectorAll('[data-zel-v47-hidden-static-bar], [data-zel-v53-hidden-static-strip], [data-zel-v54-hidden-strip], [data-zel-v55-hidden-strip]')).forEach(function (el) {
      if (!el.closest('.zel-v56-spark-wrap')) el.setAttribute('data-zel-v56-hidden-strip', '1');
    });
  }
  ...
  function render(card, sym) {
    ...
    card.classList.add('zel-v56-live-card');
    card.setAttribute('data-zel-v56-symbol', sym);
    var s = state[sym];
    ...
    if (finite(s.price)) {
      ...
      title.classList.add('zel-v56-has-live-price');
    }
    var age = s.ts ? Math.max(0, now() - s.ts) : null;
    /* UI request: do not render LIVE REST/WS status text. Keep data in state only. */
    status.textContent = '';
    status.setAttribute('aria-hidden', 'true');
    status.style.setProperty('display', 'none', 'important');
    card.classList.toggle('zel-v56-feed-live', !!live);
    card.classList.toggle('zel-v56-feed-wait', !live);
  ...
  function boot() {
    if (document.documentElement.classList.contains('zel-v56-live-core-loaded')) return;
    document.documentElement.classList.add('zel-v56-live-core-loaded');
    window.ZEL_APP_V56_LIVE_FEED = { version: VER, state: state, mount: mountAll, ingest: ingest };
    startWs();
    restFallback();
    mountAll();
