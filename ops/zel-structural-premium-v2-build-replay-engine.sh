...
# Remove every legacy overlay/no-trend post-main wrapper and rebuild one explicit V2 registry wrapper.
...
# Add pending-entry state to the lane. A signal at bar t is never opened on bar t.
...
# Replace same-bar opening with queue-only logic. producer.valid_entry is the sole new-entry predicate.
...
# Preserve execution provenance in every closed trade row (price exit + strategy exit).
...
# Contract assertions: no old overlay, no direct same-bar make_position in no-position branch.
if '_ZEL_OVERLAY' in text or 'ZEL_STRUCTURAL_PREMIUM_OVERLAY_PATCH' in text:
    raise SystemExit('LEGACY_OVERLAY_REMAINS')
if text.count('V2_ENTRY_EXECUTION_MODEL') < 2:
    raise SystemExit('V2_EXECUTION_MARKER_MISSING')
if text.count('pending_entry = {') != 1:
    raise SystemExit('PENDING_QUEUE_COUNT')
if text.count('execution_price = float(last["open"])') != 1:
