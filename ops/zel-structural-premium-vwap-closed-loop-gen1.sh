...
for cid,p in rows.items():
    ...
    out={'schema_version':'zel.structural_premium.overlay.v1','candidate_id':'VWAP_GEN1_'+cid,'generation':1,'seed':p['seed'],'changed_axis':p['changed_axis'],'parameters':params,'research_only':True,'selection_authority':False,'promotion_authority':False,'execution_authority':'NONE','order_authority':'BLOCKED'}
    out['overlay_sha256']=hashlib.sha256(json.dumps(out,sort_keys=True,separators=(',',':'),allow_nan=False).encode()).hexdigest()
    (root/(cid+'.json')).write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)+'\n')
print(json.dumps({'state':'PASS_GEN1_CANDIDATES','candidate_ids':sorted(rows)},sort_keys=True))
PYCANDS
...
run_window() {
  cid="$1"; phase="$2"; window="$3"; run="$GEN/runs/$cid"; lane="$run/engine/lane_${phase}.py"; out="$run/replay_${phase}"
  if [ ! -s "$lane" ]; then
...
spec=importlib.util.spec_from_file_location('score_'+cid+'_'+phase,engine); e=importlib.util.module_from_spec(spec); sys.modules[spec.name]=e; spec.loader.exec_module(e)
rows=[]; files=sorted((lane_root/'vwap_revert').glob('*.json.gz'))
for p in files:
  with gzip.open(p,'rt',encoding='utf-8') as h: d=json.load(h)
  if str(d.get('window_id'))!=window: raise SystemExit(f'WINDOW_LEAK:{p}:{d.get("window_id")}')
