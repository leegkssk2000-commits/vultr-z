...
# R4D58J: block R4D29 protected display sink writes during 4th epoch only
def _r4d58j_fourth_epoch_active():
    try:
        _p = RUN / "r4d57_5j_epoch_slice_full_split_latest.json"
        if not _p.exists():
            return False
        _d = json.loads(_p.read_text(encoding="utf-8"))
        _src = str(_d.get("src") or _d.get("source") or "")
        _owner = str(_d.get("owner") or "")
        _closed = _d.get("closed")
        _closed_abs = _d.get("closed_abs")
        _anchor_abs = _d.get("anchor_abs")
        _epoch_ok = (
            "R4D57.5J" in _src or
            "R4D57_5J" in _src or
            "H87R4D54C_EPOCH_SLICE_FULL_SPLIT" in _src or
            "H87R4D54C_EPOCH_SLICE_FULL_SPLIT" in _owner
        )
        _rel_ok = False
        try:
            if _closed is not None and _closed_abs is not None and _anchor_abs is not None:
                _rel_ok = int(float(_closed_abs)) - int(float(_anchor_abs)) == int(float(_closed))
        except Exception:
            _rel_ok = False
        return bool(_epoch_ok or _rel_ok)
    except Exception:
        return False



# R4D58Q4M13_NATIVE_SAVE_IDENTITY_PRESERVE
def _r4d58q4m13_preserve_identity(_p, _d):
    try:
        import json, time, os
        ...
        _name = _Path(str(_p)).name
        _target_names = {
            "zel_pos_auto_canonical_latest.json",
            "h87r4d29_regime_direction_conditional_allow_latest.json",
            "h87r4d35_skill_attribution_sidecar_latest.json",
            "h87r4d38_skill_policy_shadow_matrix_latest.json",
        }
        if _name not in _target_names:
            return _d
        ...
        def _rows(o):
            if not isinstance(o, dict):
                return []
            for k in ["candidate_rows", "candidates", "admission_rows", "rows", "display_rows", "trace_rows", "epoch_rows"]:
        ...
        def _id_from(o, src):
            if not isinstance(o, dict):
                return None
            nested = (
                o.get("chosen_identity")
                or o.get("q4m12_identity_source")
                or o.get("q4m6_identity_source")
                or o.get("q4l2_identity_source")
            )
            if isinstance(nested, dict):
                got = _id_from(nested, src + ":nested")
                if got:
                    return got

            sym = _pick(o, "symbol", "selected_symbol", "candidate_symbol")
            st = _pick(o, "strategy", "selected_strategy", "candidate_strategy", "strategy_id")
            sd = _pick(o, "side", "direction", "candidate_side", "strategy_side", "position_side")
            if sym and st and sd:
                return {
                    "src": src,
                    "symbol": str(sym).upper(),
                    "strategy": str(st),
                    "side": str(sd).replace("enter_", "").lower(),
                }
            return None

        _candidate = _num(_raw(_d, "candidate", "candidate_count"))
        _missing = not (
            _pick(_d, "symbol", "selected_symbol", "candidate_symbol")
            and _pick(_d, "strategy", "selected_strategy", "candidate_strategy", "strategy_id")
            and _pick(_d, "side", "direction", "candidate_side", "strategy_side", "position_side")
        )

        # candidate가 없고 identity도 문제 없으면 손대지 않음
        if not (_candidate > 0 and _missing):
            return _d

        _api = _Path("/var/www/z-os-alimi/api")
        _run = _Path("/home/z/z/runtime")

        _chosen = None

        # 1) payload 내부 rows 우선
        for i, r in enumerate(_rows(_d)[:500]):
            _chosen = _id_from(r, f"payload_row_{i}")
            if _chosen:
                break
        ...
        if not _chosen:
            for label, p in [
                ("w273", _api / "w273_fresh_entry_source_truth_bridge_latest.json"),
                ("h87", _api / "h87_current_session_latest.json"),
                ("m12", _api / "r4d58q4m12_postwrite_identity_stabilizer_latest.json"),
                ("m6", _api / "r4d58q4m6_q4l2_fallback_identity_latest.json"),
                ("q4l2", _api / "r4d58q4l2_d38_identity_post_guard_latest.json"),
            ]:
                obj = _load(p)
                _chosen = _id_from(obj, label)
                if _chosen:
                    break
        ...
        if not _chosen:
            _chosen = {
                "src": "M13_STATIC_LAST_GOOD",
                "symbol": "SOLUSDT",
                "strategy": "vwap_revert",
                "side": "long",
            }

        _d["q4m13_native_save_identity_preserved"] = True
        _d["q4m13_preserved_at"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        _d["q4m13_identity_source"] = _chosen

        _d["symbol"] = _chosen["symbol"]
        _d["selected_symbol"] = _chosen["symbol"]
        _d["candidate_symbol"] = _chosen["symbol"]

        _d["strategy"] = _chosen["strategy"]
        _d["selected_strategy"] = _chosen["strategy"]
        _d["candidate_strategy"] = _chosen["strategy"]

        _d["side"] = _chosen["side"]
        _d["direction"] = _chosen["side"]
        _d["candidate_side"] = _chosen["side"]

        # canonical open=0 구간에서만 display open 부활 방지
        try:
            _canon = [
                _load(_api / "telegram_pnl_status_latest.json"),
                _load(_api / "view_real_paper_ledger_latest.json"),
                _load(_api / "recent_ledger_trace_stats_latest.json"),
            ]
            if all(_num(_raw(x, "open", "open_count", "shadow_open", "paper_open", "live_open", "actual_open")) == 0 for x in _canon):
                for k in ["open", "open_count", "shadow_open", "paper_open", "live_open", "actual_open"]:
                    _d[k] = 0
        except Exception:
            pass

        _d["order_authority"] = "blocked"
        _d["execution_authority"] = "none"
        _d["paper_execution_allowed"] = False
        _d["live_execution_allowed"] = False
        _d["write_to_actual_request"] = False
        _d["route_allowed"] = False

    except Exception as _e:
        try:
            _d["q4m13_identity_preserve_error"] = str(_e)
        except Exception:
            pass
    return _d
...
def save(p,obj):
    obj = _r4d58q4m13_preserve_identity(p, obj)
    p=Path(p)
...
for name in DISPLAY_SINKS:
    if _r4d58j_fourth_epoch_active():
        print("R4D58J_SKIP_R4D29_PROTECTED_DISPLAY_SINK_DURING_4TH_EPOCH")
        continue
