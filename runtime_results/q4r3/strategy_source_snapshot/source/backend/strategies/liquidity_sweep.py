...
class LiquiditySweepConfig:
    ...
    upper_sweep_wick_atr: float = 0.55
    lower_sweep_wick_atr: float = 0.55
    reclaim_atr_min: float = 0.12
    max_chase_dist_atr: float = 1.20
    fail_sweep_break_atr: float = 0.22

    short_rsi_min: float = 55.0
    long_rsi_max: float = 45.0
    beam_short_rsi_min: float = 62.0
    beam_long_rsi_max: float = 38.0

    stop_atr_mult: float = 0.30
    trail_atr_mult: float = 0.22
    base_rr: float = 2.00
    beam_rr: float = 2.60
...
def strategy(
    ...
    *,
    ...
    config: Optional[LiquiditySweepConfig] = None,
) -> Dict[str, Any]:
    ...
    upper_sweep = (
        ...
        and upper_wick > atr_now * cfg.upper_sweep_wick_atr
    )
    lower_sweep = (
        low < swing_low
        and price > swing_low
        and lower_wick > atr_now * cfg.lower_sweep_wick_atr
    )

    short_reclaim = price < prev_close - atr_now * cfg.reclaim_atr_min
    long_reclaim = price > prev_close + atr_now * cfg.reclaim_atr_min

    short_setup = upper_sweep and rsi_now > cfg.short_rsi_min and short_reclaim and not trend_long
    long_setup = lower_sweep and rsi_now < cfg.long_rsi_max and long_reclaim and not trend_short
    ...
    failed_long = price < swing_low - atr_now * cfg.fail_sweep_break_atr
    failed_short = price > swing_high + atr_now * cfg.fail_sweep_break_atr
    ...
    if not vol_ok:
        return _build_result(
            ...
            tp=price,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_volatility_out_of_range",
            skill="none",
            confidence=0.0,
    ...
    if late_chase_block and (long_setup or short_setup):
        return _build_result(
            ...
            tp=price,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_late_chase_block",
            skill="none",
            confidence=0.0,
    ...
    long_sl = min(low - atr_now * cfg.stop_atr_mult, swing_low - atr_now * cfg.trail_atr_mult)
    short_sl = max(high + atr_now * cfg.stop_atr_mult, swing_high + atr_now * cfg.trail_atr_mult)
    ...
    if in_long and can_add_more:
        long_add = lower_sweep and long_reclaim and not failed_long
        long_reduce = failed_long

    if in_short and can_add_more:
        short_add = upper_sweep and short_reclaim and not failed_short
    ...
    if short_setup and not in_long and not in_short:
        ...
        return _build_result(
            ...
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_short",
            skill="short_beam" if short_beam else "stop_run_reject",
            confidence=0.82 if short_beam else 0.68,
            tags=["liquidity", "sweep", "short"],
            indicators=indicators,
        )
    ...
    if long_setup and not in_long and not in_short:
        ...
        return _build_result(
            ...
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_long",
            skill="long_beam" if long_beam else "stop_run_reject",
            confidence=0.84 if long_beam else 0.70,
            tags=["liquidity", "sweep", "long"],
            indicators=indicators,
        )
    ...
    if long_add:
        return _build_result(
            ...
            tp=long_tp,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_long_add",
            skill="dip_add",
            confidence=0.58,
            tags=["liquidity", "sweep", "add", "long"],
            indicators=indicators,
        )
    ...
    if short_add:
        return _build_result(
            ...
            tp=short_tp,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_short_add",
            skill="dip_add",
            confidence=0.54,
            tags=["liquidity", "sweep", "add", "short"],
            indicators=indicators,
        )
    ...
    if long_reduce:
        return _build_result(
            ...
            tp=long_tp,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_failed_long_reduce",
            skill="failed_sweep_reduce",
            confidence=0.68,
            tags=["liquidity", "sweep", "failed", "reduce", "long"],
            indicators=indicators,
        )
    ...
    if short_reduce:
        return _build_result(
            ...
            tp=short_tp,
            pyramiding=cfg.max_pyramiding,
            why="liq_sweep_failed_short_reduce",
            skill="failed_sweep_reduce",
            confidence=0.64,
            tags=["liquidity", "sweep", "failed", "reduce", "short"],
            indicators=indicators,
        )

    hold_reason = "liq_sweep_no_setup"
    if upper_sweep and rsi_now <= cfg.short_rsi_min:
        hold_reason = "upper_sweep_without_short_rsi"
    elif lower_sweep and rsi_now >= cfg.long_rsi_max:
        hold_reason = "lower_sweep_without_long_rsi"
...
class LiquiditySweepLBotStrategy(LBotStrategyBase):
    strategy_name = "liquidity_sweep"
    ...
    def decide(self, ctx: DecisionContext) -> StrategyDecision:
        ...
        reason = str(result.get("why") or "liq_sweep_no_reason")
