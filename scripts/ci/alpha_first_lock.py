...
EXPECTED_ENGINE_ORDER = [
    "market_data", "alpha_engine", "regime_router", "entry_qualifier", "risk_exit",
    "execution", "portfolio", "bots_skills_advisor", "shadow", "paper", "live",
]
EXPECTED_ALPHA_FAMILIES = ["trend_momentum", "carry_flow", "relative_value_psa"]
...
def validate_policy(policy: Mapping[str, object]) -> list[str]:
    ...
    g0 = policy.get("g0_installation_certification")
    if not isinstance(g0, Mapping) or g0.get("required_before_destructive_cleanup") is not True:
        errors.append("G0 certification must precede destructive cleanup")
    return errors
...
def git_changed_files(base: str, head: str) -> list[str]:
    proc = subprocess.run(
        ["git", "diff", "--name-only", f"{base}...{head}"], cwd=ROOT, check=True,
