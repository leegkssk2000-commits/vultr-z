...
SENSITIVE_EXACT_KEYS = {
    "account", "account_id", "api_key", "apikey", "credential", "credentials",
    "exchange_key", "order", "orders", "order_id", "password", "position",
    "positions", "position_id", "private_key", "secret", "token", "wallet",
}
AUTHORITY_ONLY_KEYS = {
...
SENSITIVE_KEY_TERMS = ("secret", "password", "credential", "private_key")
SENSITIVE_VALUE_MARKERS = ("bearer ", "-----begin private key-----", "sk-", "gsk_", "cfut_")
...
def assert_anonymized(value: Any, path: str = "$") -> None:
    """Reject real sensitive/order-bearing fields without substring false positives."""
    if isinstance(value, dict):
        for raw_key, child in value.items():
            key = str(raw_key).strip().lower()
            if key in AUTHORITY_ONLY_KEYS:
                validate_authority_field(key, child, path)
            elif key in SENSITIVE_EXACT_KEYS or any(term in key for term in SENSITIVE_KEY_TERMS):
...
def load_payload(input_path: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": {"axis": "TREND_REGIME_GATE", "generation": 1, "change": "Apply one pre-entry trend-regime eligibility gate."},
        "evidence": {"trades": 24, "retention_pct": 87.5, "positive_windows": 2, "ordered_marginal_delta_net": {"TEAM": 0.4}},
...
def build_prompt(payload: dict[str, Any], *, retry: bool = False) -> str:
    prefix = "Your previous answer violated the JSON contract. Return exactly one valid JSON object, no markdown or prose.\n" if retry else ""
    return (
        prefix
...
def request_review(client: Groq, model: str, payload: dict[str, Any]) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    for attempt in range(MAX_JSON_ATTEMPTS):
        ...
        completion = client.chat.completions.create(model=model, temperature=0, max_tokens=512, messages=[{"role": "user", "content": prompt}])
...
def main() -> int:
    ...
    base_artifact: dict[str, Any] = {
        "schema_version": "strategy11.groq_redteam.v2.0",
        "provider": "groq",
    ...
    try:
        ...
        artifact = {
            **base_artifact,
            ...
            "semantic_input_sha": sha256_text(semantic_text),
