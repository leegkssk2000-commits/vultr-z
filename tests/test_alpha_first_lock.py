...
class AlphaFirstLockTests(unittest.TestCase):
    ...
    def test_policy_has_exact_three_families(self):
        self.assertEqual([], MODULE.validate_policy(self.policy))
        self.assertEqual(
            ["trend_momentum", "carry_flow", "relative_value_psa"],
            self.policy["alpha_engine"]["allowlist"],
        )

    def test_zero_survivor_allows_alpha_research(self):
    ...
    def test_nonpositive_edge_holds(self):
        ...
        self.assertEqual(
            ["oos_net_pnl_positive", "oos_expectancy_positive"],
            failures,
        )
