...
def test_negative_net_value_fails_closed() -> None:
    rows = tuple(attribution.ProviderOutcome(**{
        **item.__dict__, "realized_r": -0.20, "baseline_r": 0.10,
...
def test_confidence_drift_fails_closed() -> None:
    ...
    current[0] = drift.QualitySnapshot(**{**current[0].__dict__, "mean_confidence": 0.99})
...
def test_schema_failure_drift_fails_closed() -> None:
    current = list(snapshots(current=True))
    current[1] = drift.QualitySnapshot(**{**current[1].__dict__, "schema_failure_rate": 0.20})
    result = drift.evaluate_quality_drift(
        snapshots(), current, expected_provider_ids=PROVIDERS,
        now_ms=10000, policy=drift_policy(),
    )
...
def test_zbot_sgrade_lock_passes_only_at_24_surfaces() -> None:
    result = sgrade.evaluate_sgrade_lock(
        ...
        attribution=ready_attribution(), drift=ready_drift(),
        observer_only=True, proposal_only=True,
        provider_invocation_enabled=False, runtime_enabled=False,
        execution_authority="none", order_authority="none",
        human_approval_required=True, same_epoch_auto_apply=False,
    )
...
def test_sgrade_lock_rejects_runtime_or_authority() -> None:
    result = sgrade.evaluate_sgrade_lock(
        ...
        attribution=ready_attribution(), drift=ready_drift(),
        observer_only=True, proposal_only=True,
        provider_invocation_enabled=True, runtime_enabled=True,
        execution_authority="write", order_authority="order",
        human_approval_required=True, same_epoch_auto_apply=False,
    )
    assert result.state == "HOLD"
...
def test_contract_closes_all_surfaces_without_runtime_enablement() -> None:
    ...
    assert contract["surface_count"] == {
        "prior_ready": 22, "closed_now": 2, "total_ready": 24, "total": 24,
    }
