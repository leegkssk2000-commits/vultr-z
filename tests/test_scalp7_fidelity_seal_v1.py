...
seal: Any = importlib.import_module("scripts.verify_scalp7_fidelity_saved_v1")
...
def publication(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(seal, "saved_checks", seal.verify_registry)
    seal.seal(tmp_path)
    return tmp_path
...
def test_frozen_dependency_paths_cannot_escape(publication: Path, unsafe: str):
    path = publication / seal.CAMPAIGN / "freezes/HG.json"
...
def test_resealing_cashflow_tampering_still_calls_unchanged_saved_arithmetic(
    ...
    frozen = {"windows": windows, "hashes": {}}
    freeze_path = seal.CAMPAIGN + "/freezes/HG.json"
    dump(publication, freeze_path, frozen)
    payload = {"trades": [row], "unresolved": [], "window_receipts": []}
    ledger_name = seal.CAMPAIGN + "/results/HG.trades.json.gz"
    ledger = publication / ledger_name
    ledger.write_bytes(gzip.compress(json.dumps(payload).encode()))
    signals_name = seal.CAMPAIGN + "/results/HG.signals.json.gz"
    info = {
    ...
    dump(publication, seal.CAMPAIGN + "/results/HG.json", info)
    ...
    monkeypatch.setattr(seal, "saved_checks", lambda _: runner.verify_saved("HG"))
...
def test_receipt_pointers_cannot_escape_sealed_alias_files(
    ...
    path = publication / seal.CAMPAIGN / "results/HG.json"
    ...
    info[key] = {
        ...
        "wrong_alias": seal.CAMPAIGN + "/freezes/RSI.json",
    }[escape]
