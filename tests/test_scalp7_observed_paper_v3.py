...
def proof(native: int, received: int, usable: int) -> dict[str, Any]:
    from backend.research.rebuild.scalp7_source_clock_v3 import await_native_time

    walls = iter([received, usable])
    monos = iter([0, max(0, usable - received) * 1_000_000])
    return await_native_time(
        native,
        received,
...
def quote(
    ...
    return {
        ...
        "usable_at_ms": stamp,
        "clock_proof": proof(stamp - 1, stamp, stamp),
...
def test_normalize_public_quote_strict_identity_nonzero_prices_and_provenance() -> None:
    ...
    q = paper.normalize_quote(
        raw, SYMBOL, BASE, BASE + 2, clock_barrier=lambda n, r: proof(n, r, r)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"bids": [[99, 1]], "asks": [[100, 1]], "T": BASE + 3, "time": BASE + 4},
    ],
)
...
def runtime_fixture(tmp_path: Path, monkeypatch: Any) -> tuple[Path, Path]:
        ...
        "code_pins": [
            {
                "path": str(
                    Path(paper.__file__)
                    .with_name("scalp7_source_clock_v3.py")
                    .resolve()
                ),
                "sha256": paper.sha_file(
                    Path(paper.__file__).with_name("scalp7_source_clock_v3.py")
                ),
            },
            {
    ...
    forward = SimpleNamespace(
        FreshForwardError=type("FixtureSnapshotError", (RuntimeError,), {}),
...
def test_runtime_requires_exact_self_path_and_matching_forward_config(
    ...
    data["code_pins"] = [
        data["code_pins"][0],
        {"path": str(fake), "sha256": paper.sha_file(fake)},
    ]
...
def test_exit_quote_included_in_mae_and_outcome_availability_is_processing_clock() -> (
    ...
    row = m.state["trades"][0]
    assert row["mae_R"] == 2
    assert row["exit_ts_ms"] == BASE + 140
    assert row["exit_quote_ts_by_leg"][SYMBOL] == BASE + 130
...
@pytest.mark.parametrize("lead", [1, 4])
def test_native_lead_requires_actual_clock_proof_without_timestamp_rewrite(
    lead: int,
) -> None:
    received = BASE + 110
    native = received + lead
    raw = json.dumps(
        {
            "code": 0,
            "data": {
    ...
    q = paper.normalize_quote(
        ...
        received - 1,
        received,
        clock_barrier=lambda n, r: proof(n, r, native),
    )
    assert q["received_at_ms"] == received
    assert q["source_ts_ms"] == native
    assert q["usable_at_ms"] == native
    assert q["clock_proof"]["quarantined"] is True
    m = machine()
    m.admit(wrapper(), BASE + 101)
    m.step({SYMBOL: q}, contexts(), native - 1)
    assert not m.state["positions"]
    m.step({SYMBOL: q}, contexts(), native)
    position = next(iter(m.state["positions"].values()))
    assert position["entry_ts_ms"] == native
    assert position["entry_available_ts_ms"] == native
    assert position["entry_quote_ts_by_leg"][SYMBOL] == received


def test_missing_or_forged_clock_proof_never_admits_quote() -> None:
    for mutate in (
        lambda q: q.pop("clock_proof"),
        lambda q: q["clock_proof"].update(usable_at_ms=q["received_at_ms"] - 1),
        lambda q: q["clock_proof"].update(clock_samples=[]),
        lambda q: q["clock_proof"].update(clock_offset_or_tolerance_applied=True),
    ):
        m = machine()
        m.admit(wrapper(), BASE + 101)
        q = quote(BASE + 110)
        mutate(q)
        with pytest.raises(paper.PaperError, match="CLOCK"):
            m.step({SYMBOL: q}, contexts(), BASE + 110)
        assert not m.state["positions"]


def test_quote_capture_preserves_raw_receipt_and_separate_clock_receipt(
    tmp_path: Path, monkeypatch: Any
) -> None:
    raw = json.dumps(
        {
    ...
    clocks = iter([BASE, BASE + 1])
    monkeypatch.setattr(paper, "wall_clock_ms", lambda: next(clocks))
    monkeypatch.setattr(paper, "await_native_time", lambda n, r: proof(n, r, BASE + 4))
    q = paper.quote_request(tmp_path, SYMBOL, lambda url: (200, raw))
    receipt = json.loads(Path(q["receipt_path"]).read_bytes())
    clock_receipt = json.loads(Path(q["clock_proof_path"]).read_bytes())
    assert receipt["requested_at_ms"] == BASE
    assert receipt["received_at_ms"] == BASE + 1
    assert Path(receipt["body_path"]).read_bytes() == raw
    assert q["source_ts_ms"] == BASE + 4
    assert q["usable_at_ms"] == BASE + 4
    assert clock_receipt["receipt_sha256"] == q["receipt_sha256"]
    assert clock_receipt["body_sha256"] == q["body_sha256"]
    assert clock_receipt["clock_proof"] == q["clock_proof"]


def test_exit_and_partial_actions_use_processing_clock_after_proof() -> None:
    m = machine()
    p = enter(m, wrapper(partial_take_profit_r=1, partial_fraction=0.1))
    m.step({SYMBOL: quote(BASE + 120, 106, 107)}, contexts(), BASE + 125)
    q = quote(BASE + 130, 104, 105)
    q.update(
        source_ts_ms=BASE + 134,
        usable_at_ms=BASE + 134,
        clock_proof=proof(BASE + 134, BASE + 130, BASE + 134),
    )
    m.step({SYMBOL: q}, contexts(), BASE + 133)
    assert p["remaining"] == 1
    m.step({SYMBOL: q}, contexts(), BASE + 140)
    assert p["partials"][0]["filled_at_ms"] == BASE + 140
    assert p["partials"][0]["quote_receipts"][SYMBOL]["received_at_ms"] == BASE + 130
    m.step({SYMBOL: quote(BASE + 150, 94, 95)}, contexts(), BASE + 150)
    q = quote(BASE + 160, 90, 91)
    q.update(
        source_ts_ms=BASE + 164,
        usable_at_ms=BASE + 164,
        clock_proof=proof(BASE + 164, BASE + 160, BASE + 164),
    )
    m.step({SYMBOL: q}, contexts(), BASE + 163)
    assert not m.state["trades"]
    m.step({SYMBOL: q}, contexts(), BASE + 170)
    closed = m.state["trades"][0]
    assert closed["exit_ts_ms"] == closed["outcome_available_ts_ms"] == BASE + 170
    assert closed["exit_quote_ts_by_leg"][SYMBOL] == BASE + 160
    assert closed["exit_quote_usable_by_leg"][SYMBOL] == BASE + 164


def publish_fixture(producer: Path) -> None:
    row = wrapper()
    row.update(
        previous_sha256="0" * 64, freeze_sha256=paper.sha_file(producer / "FREEZE.json")
    )
...
def test_consumed_missing_signal_projection_holds_before_quote_and_cursor_advance(
    tmp_path: Path, monkeypatch: Any
) -> None:
    cfg, producer = runtime_fixture(tmp_path, monkeypatch)
    out = tmp_path / "paper"
    paper.initialize(out, cfg, BASE - 1)
    publish_fixture(producer)
    calls: list[int] = []

    def capture(*args: Any) -> dict[str, Any]:
        calls.append(1)
        return quote(BASE + 110)

    monkeypatch.setattr(paper, "quote_request", capture)
    paper.poll(out, cfg, now_ms=BASE + 101)
    saved = (out / "STATE.json").read_bytes()
    (producer / "fresh_signals.jsonl").unlink()
    with pytest.raises(
        paper.PaperError, match="CONSUMED_SIGNAL_PROJECTION_MISSING_HOLD"
    ):
        paper.poll(out, cfg, now_ms=BASE + 120)
    assert (out / "STATE.json").read_bytes() == saved
    assert calls == [1]
    assert paper.read_signal_projection(tmp_path / "not_created.jsonl", {}) == ([], {})


@pytest.mark.parametrize("fail_phase", [1, 2])
def test_exact_snapshot_change_retries_without_any_signal_cursor_or_state_advance(
    tmp_path: Path, monkeypatch: Any, fail_phase: int
) -> None:
    cfg, producer = runtime_fixture(tmp_path, monkeypatch)
    out = tmp_path / "paper"
    paper.initialize(out, cfg, BASE - 1)
    paper.poll(out, cfg, now_ms=BASE)
    saved = (out / "STATE.json").read_bytes()
    publish_fixture(producer)
    forward: Any = paper.importlib.import_module(
        "backend.research.rebuild.scalp7_fresh_forward_v2"
    )
    build = forward.build_current_frames
    phases: list[int] = []

    def snapshot(*args: Any, **kwargs: Any) -> Any:
        phases.append(1)
        if len(phases) == fail_phase:
            raise forward.FreshForwardError(
                "SOURCE_SNAPSHOT_CHANGED_RETRY_WITHOUT_ADVANCING"
            )
        return build(*args, **kwargs)

    forward.build_current_frames = snapshot
    quotes = iter([quote(BASE + 110), quote(BASE + 120)])
    monkeypatch.setattr(paper, "quote_request", lambda *args: next(quotes))
    retry = paper.poll(out, cfg, now_ms=BASE + 101)
    assert retry["state"] == "RETRY_SOURCE_SNAPSHOT_CHANGED_NO_STATE_ADVANCE"
    assert retry["state_or_signal_cursor_advanced"] is False
    assert (out / "STATE.json").read_bytes() == saved
    forward.build_current_frames = build
    result = paper.poll(out, cfg, now_ms=BASE + 111)
    assert result["open_positions"] == 1
    state = paper._load_state(out, paper.sha_file(out / "FREEZE.json"))
    assert len(state["seen"]) == len(state["positions"]) == 1


def test_other_snapshot_errors_are_not_retried_as_exact_transient(
    tmp_path: Path, monkeypatch: Any
) -> None:
    cfg, _ = runtime_fixture(tmp_path, monkeypatch)
    forward: Any = paper.importlib.import_module(
        "backend.research.rebuild.scalp7_fresh_forward_v2"
    )
    for error in [
        forward.FreshForwardError(
            "SOURCE_SNAPSHOT_CHANGED_RETRY_WITHOUT_ADVANCING_EXTRA"
        ),
        RuntimeError("SOURCE_SNAPSHOT_CHANGED_RETRY_WITHOUT_ADVANCING"),
    ]:

        def failing(*args: Any, **kwargs: Any) -> Any:
            raise error

        monkeypatch.setattr(paper, "_poll_once", failing)
        with pytest.raises(type(error), match=str(error)):
            paper.poll(tmp_path / "paper", cfg, now_ms=BASE)


def test_actual_final_processing_clock_is_sampled_after_frame_refresh(
    tmp_path: Path, monkeypatch: Any
) -> None:
    cfg, producer = runtime_fixture(tmp_path, monkeypatch)
    out = tmp_path / "paper"
    ...
    def build(*args: Any, **kwargs: Any) -> Any:
        phase.append("refresh" if kwargs.get("bind") is False else "initial")
        return contexts(), contexts(), {}

    forward.build_current_frames = build
    clock_values = iter([BASE + 101, BASE + 102, BASE + 110, BASE + 150])

    def clock() -> int:
        value = next(clock_values)
        if value == BASE + 150:
            assert phase == ["initial", "refresh"]
        return value

    monkeypatch.setattr(paper, "wall_clock_ms", clock)
    monkeypatch.setattr(paper, "quote_request", lambda *args: quote(BASE + 110))
    paper.poll(out, cfg)
    state = paper._load_state(out, paper.sha_file(out / "FREEZE.json"))
    p = next(iter(state["positions"].values()))
    assert p["entry_ts_ms"] == p["entry_available_ts_ms"] == BASE + 150
    assert p["entry_quote_ts_by_leg"][SYMBOL] == BASE + 110


def test_runtime_requires_shared_actual_clock_source_pin(
    tmp_path: Path, monkeypatch: Any
) -> None:
    cfg, _ = runtime_fixture(tmp_path, monkeypatch)
    data = json.loads(cfg.read_bytes())
    data["code_pins"] = [
        x
        for x in data["code_pins"]
        if not x["path"].endswith("scalp7_source_clock_v3.py")
    ]
    cfg.write_bytes(paper.json_bytes(data))
    with pytest.raises(paper.PaperError, match="SHARED_ACTUAL_CLOCK_PIN_REQUIRED"):
        paper.runtime_config(cfg)


def test_pair_all_legs_need_proof_and_usable_times_must_obey_skew() -> None:
    m = machine()
    row = wrapper(
        symbol="BTC-USDT|ETH-USDT",
        side=0,
        stop_price=None,
        legs=[
            {"symbol": SYMBOL, "side": 1, "weight": 0.5},
            {"symbol": "ETH-USDT", "side": -1, "weight": 0.5},
        ],
    )
    m.admit(row, BASE + 101)
    q1 = quote(BASE + 110)
    q2 = quote(BASE + 110, 200, 201, "ETH-USDT")
    q2.update(
        source_ts_ms=BASE + 124,
        usable_at_ms=BASE + 124,
        clock_proof=proof(BASE + 124, BASE + 110, BASE + 124),
    )
    qs = {SYMBOL: q1, "ETH-USDT": q2}
    m.step(qs, contexts(), BASE + 123)
    assert not m.state["positions"]
    m.step(qs, contexts(), BASE + 124)
    assert not m.state["positions"]
    q1.update(
        source_ts_ms=BASE + 124,
        usable_at_ms=BASE + 124,
    ...
    m.step(qs, contexts(), BASE + 124)
    p = next(iter(m.state["positions"].values()))
    assert p["entry_ts_ms"] == BASE + 124
    assert p["entry_quote_ts_by_leg"] == {SYMBOL: BASE + 110, "ETH-USDT": BASE + 110}


def test_processing_clock_reversal_after_snapshot_holds_without_state_commit(
    tmp_path: Path, monkeypatch: Any
) -> None:
    cfg, producer = runtime_fixture(tmp_path, monkeypatch)
    out = tmp_path / "paper"
    paper.initialize(out, cfg, BASE - 1)
    publish_fixture(producer)
    clocks = iter([BASE + 101, BASE + 102, BASE + 110, BASE + 105])
    monkeypatch.setattr(paper, "wall_clock_ms", lambda: next(clocks))
    monkeypatch.setattr(paper, "quote_request", lambda *args: quote(BASE + 110))
    with pytest.raises(
        paper.PaperError, match="LOCAL_CLOCK_REVERSED_AFTER_QUOTE_SNAPSHOT"
    ):
        paper.poll(out, cfg)
    assert not (out / "STATE.json").exists()
