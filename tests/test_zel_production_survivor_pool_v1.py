...
def survivor(family: str, score: float, *, suffix: str = "a", dd: float = 5.0) -> dict:
    return {
...
def test_distinct_family_top3_plus2_and_target_event() -> None:
    rows = [survivor(f"family{i}", float(10 - i)) for i in range(5)]
    # Same family, lower-ranked variant must never consume a second slot.
    rows.append(survivor("family0", 1.0, suffix="worse"))
...
def test_pool_change_event_rotates_active_when_rank_changes() -> None:
    ...
    second_rows = [survivor(f"f{i}", float(5-i)) for i in range(4)] + [survivor("new", 99.0)]
...
def test_legacy_incumbent_is_ingested_as_one_family() -> None:
    registry = {
