...
BRANCH = "q4r3-exact25-r71-skill-adjusted-shadow-matrix-contract-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_exact25_r71_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r71_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run(
            [
                python_bin(root),
                str(worktree / "tools/run_q4r3_exact25_r71_skill_adjusted_shadow_matrix.py"),
                "--root",
                str(root),
                "--worktree",
                str(worktree),
        ...
        status = root / "runtime/exact25_edge_v1/exact25_r71_skill_adjusted_shadow_matrix_contract/status_latest.json"
        evidence = worktree / "evidence/q4r3_exact25_r71_skill_adjusted_shadow_matrix_contract_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run(
                [
                    ...
                    "commit",
                    "-m",
                    "Record R7.1 Exact25 skill-adjusted matrix evidence",
                ],
                cwd=worktree,
            )
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_EXACT25_R71_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R7.1")
        ...
        print("R64_EXTERNAL_CANARY_REQUIRED=false")
        print("EVIDENCE=evidence/q4r3_exact25_r71_skill_adjusted_shadow_matrix_contract_latest.json")
        passed = True
        return 0
    finally:
