...
BRANCH = "q4r3-exact25-r73b1-single-owner-quarantine-plan-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_exact25_r73b1v2_{args.sha[:12]}")
    runtime = root / "runtime/exact25_edge_v1/exact25_r73b1_single_owner_plan"
    ...
    protected = {
        ...
        "R73B0_INVENTORY": root / "runtime/exact25_edge_v1/exact25_r73b0_display_binding_residue_audit/inventory_latest.json",
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r73b1v2_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root), str(worktree / "tools/run_q4r3_exact25_r73b1_single_owner_plan_v2.py"),
        ...
        if payload.get("state") != "PASS" or payload.get("blockers"):
            raise RuntimeError(f"R73B1_STATUS_NOT_PASS:{payload}")
        evidence_status = worktree / "evidence/q4r3_exact25_r73b1_single_owner_plan_latest.json"
        evidence_plan = worktree / "evidence/q4r3_exact25_r73b1_single_owner_disposition_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record corrected R7.3B1 canonical-unit evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)
        print("Q4R3_EXACT25_R73B1_CANONICAL_UNIT_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R7.3B1")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print(f"RAW_WRITER_CANDIDATE_COUNT={payload.get('raw_writer_candidate_count', 0)}")
        print(f"CANONICAL_WRITER_UNIT_COUNT={payload.get('canonical_writer_unit_count', 0)}")
        print(f"PRESERVE_CORE_RUNTIME_COUNT={payload.get('preserve_core_runtime_count', 0)}")
        print(f"PRESERVE_MEASUREMENT_WRITER_COUNT={payload.get('preserve_measurement_writer_count', 0)}")
        print(f"PRESERVE_READ_ONLY_CONSUMER_COUNT={payload.get('preserve_read_only_consumer_count', 0)}")
        print(f"PLANNED_ISOLATION_COUNT={payload.get('planned_isolation_count', 0)}")
        print(f"STATIC_LOCK_COUNT={payload.get('static_lock_count', 0)}")
        print("CLEANUP_APPLIED=false")
        print("MUTATION_AUTHORITY=none")
        print("EVIDENCE=evidence/q4r3_exact25_r73b1_single_owner_plan_latest.json")
        passed = True
        return 0
    finally:
