...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_exact25_r73b1_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r73b1_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root), str(worktree / "tools/run_q4r3_exact25_r73b1_single_owner_plan.py"),
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R7.3B1 single-owner disposition evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)
        print("Q4R3_EXACT25_R73B1_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R7.3B1")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print(f"FUTURE_OWNER_COUNT={payload.get('future_owner_count', 0)}")
