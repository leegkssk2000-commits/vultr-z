...
BRANCH = "q4r3-exact25-r73b4t-explicit-binding-canary-v1"
CORE_UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_exact25_r73b4t_{args.sha[:12]}")
    runtime = root / "runtime/exact25_edge_v1/exact25_r73b4t_explicit_binding_canary"
    status = runtime / "status_latest.json"
    before = pids()
    if any(value in {"", "0"} for value in before.values()):
        print(f"R73B4T_HOLD=CORE_PID_INVALID:{before}")
        return 2
    if worktree.exists():
    ...
    tests = subprocess.run([
        py, "-m", "pytest", "-q",
        str(worktree / "tests/test_q4r3_exact25_r73b4t_explicit_binding_canary.py"),
    ], check=False)
    if tests.returncode != 0:
        print("R73B4T_HOLD=TEST_FAILURE")
    ...
    if not status.is_file():
        print(f"R73B4T_HOLD=STATUS_MISSING:rc={run.returncode}")
        return 2
    payload = json.loads(status.read_text(encoding="utf-8"))
    after = pids()
    if after != before:
        print(f"R73B4T_HOLD=CORE_PID_CHANGED:before={before}:after={after}")
        return 2
    evidence = worktree / "evidence/q4r3_exact25_r73b4t_explicit_binding_canary_latest.json"
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
        subprocess.run([
            ...
            "Record R7.3B4T direct display binding evidence",
        ], cwd=worktree, check=True)
        subprocess.run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree, check=True)
    subprocess.run(["git", "-C", str(root), "worktree", "remove", "--force", str(worktree)], check=False)
    print("Q4R3_EXACT25_R73B4T_BOOTSTRAP_COMPLETE")
    print("OFFICIAL_STAGE=R7.3B4T")
    ...
    print(f"ALIMI_CLOSED_COUNT={payload.get('endpoint_closed_count')}")
    print(f"ALIMI_PNL_R={payload.get('endpoint_pnl_r')}")
    print(f"TELEGRAM_CLOSED_COUNT={payload.get('telegram_closed_count')}")
    print(f"TELEGRAM_PNL_R={payload.get('telegram_pnl_r')}")
    print(f"TELEGRAM_SOURCE_REPLACEMENT_COUNT={payload.get('telegram_source_replacement_count', 0)}")
    ...
    print("EVIDENCE=evidence/q4r3_exact25_r73b4t_explicit_binding_canary_latest.json")
