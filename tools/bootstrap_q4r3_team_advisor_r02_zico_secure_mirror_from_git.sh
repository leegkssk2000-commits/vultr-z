...
SHA="${Q4R3_R02_SHA:?Q4R3_R02_SHA_REQUIRED}"
TARGET_BRANCH="${Q4R3_TARGET_BRANCH:-q4r3-team-advisor-r02-zico-secure-mirror-v1}"
WT="/tmp/q4r3_team_advisor_r02_zico_${SHA:0:12}"
...
cleanup() {
  ...
  if [[ "$code" -eq 0 ]]; then
    ...
    echo "R02_WORKTREE_PRESERVED_FOR_DIAGNOSIS=$WT"
  fi
}
trap cleanup EXIT
...
Q4R3_ROOT="$ROOT" Q4R3_WORKTREE="$WT" \
  bash "$WT/tools/run_q4r3_team_advisor_r02_zico_secure_mirror.sh"

Q4R3_ROOT="$ROOT" Q4R3_WORKTREE="$WT" Q4R3_TARGET_BRANCH="$TARGET_BRANCH" \
  bash "$WT/tools/publish_q4r3_team_advisor_r02_zico_secure_mirror.sh"

echo Q4R3_TEAM_ADVISOR_R02_ZICO_SECURE_MIRROR_AND_PUBLISH_COMPLETE
