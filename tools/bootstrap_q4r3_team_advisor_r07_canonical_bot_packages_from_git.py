...
BRANCH = "q4r3-team-advisor-r07-canonical-bot-packages-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r07_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r07_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r07_canonical_bot_packages.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r07_canonical_bot_packages/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r07_canonical_bot_packages_latest.json"
        ...
        if changed:
            run([
                ...
                "commit", "-m", "Record R0.7 canonical Bot package evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)
        print("Q4R3_TEAM_ADVISOR_R07_BOOTSTRAP_PASS")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r07_canonical_bot_packages_latest.json")
        passed = True
        return 0
    finally:
