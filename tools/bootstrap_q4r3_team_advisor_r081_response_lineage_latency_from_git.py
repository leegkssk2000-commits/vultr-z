...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r081_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r081_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r081_response_lineage_latency.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r081_response_lineage_latency/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r081_response_lineage_latency_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R0.8.1 response lineage latency evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R081_BOOTSTRAP_PASS")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r081_response_lineage_latency_latest.json")
        passed = True
        return 0
    finally:
