...
BRANCH = "q4r3-team-advisor-r082-role-authority-lock-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r082_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r082_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r082_role_authority.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r082_role_authority/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r082_role_authority_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R0.8.2 Team role authority evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R082_BOOTSTRAP_PASS")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r082_role_authority_latest.json")
        passed = True
        return 0
    finally:
