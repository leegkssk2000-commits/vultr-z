...
BRANCH = "q4r3-team-advisor-r09-team-proposal-attribution-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r09_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r09_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([python_bin(root), str(wt / "tools/run_q4r3_team_advisor_r09_team_proposal_attribution.py"), "--root", str(root), "--worktree", str(wt)])
        after = unit_pids()
        if after != before:
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r09_team_proposal_attribution/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r09_team_proposal_attribution_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run(["git", "-c", "user.name=ZEL Runtime Evidence", "-c", "user.email=zel-runtime-evidence@localhost", "commit", "-m", "Record R0.9 Team proposal attribution evidence"], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)
        print("Q4R3_TEAM_ADVISOR_R09_BOOTSTRAP_PASS")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r09_team_proposal_attribution_latest.json")
        passed = True
        return 0
    finally:
