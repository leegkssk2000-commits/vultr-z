...
BRANCH = "q4r3-team-advisor-r11-zlice-formal-ledger-outcome-join-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r11_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r11_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root), str(wt / "tools/run_q4r3_team_advisor_r11_formal_ledger_outcome_join.py"),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r11_formal_ledger_outcome_join/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r11_formal_ledger_outcome_join_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R1.1 Formal Ledger outcome join evidence"
            ], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)
        print("Q4R3_TEAM_ADVISOR_R1_1_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R1.1")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r11_formal_ledger_outcome_join_latest.json")
        passed = True
        return 0
    finally:
