...
BRANCH = "q4r3-team-advisor-r21-bot-sgrade-revalidation-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r21_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r21_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([python_bin(root), str(wt / "tools/run_q4r3_team_advisor_r21_bot_sgrade_revalidation.py"), "--root", str(root), "--worktree", str(wt)])
        after = unit_pids()
        if after != before:
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r21_bot_sgrade_revalidation/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r21_bot_sgrade_revalidation_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run(["git", "-c", "user.name=ZEL Runtime Evidence", "-c", "user.email=zel-runtime-evidence@localhost", "commit", "-m", "Record R2.1 Bot readiness evidence"], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)
        print("Q4R3_TEAM_ADVISOR_R21_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R2.1")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r21_bot_sgrade_revalidation_latest.json")
        passed = True
        return 0
    finally:
