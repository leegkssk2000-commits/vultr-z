...
BRANCH = "q4r3-team-advisor-r24-mbot-sgrade-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r24_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r24_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(wt / "tools/run_q4r3_team_advisor_r24_mbot_sgrade.py"),
            "--root", str(root),
            "--worktree", str(wt),
        ])
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r24_mbot_sgrade/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r24_mbot_sgrade_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R2.4 MBot S-grade evidence",
            ], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)

        print("Q4R3_TEAM_ADVISOR_R24_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R2.4")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r24_mbot_sgrade_latest.json")
        passed = True
        return 0
    finally:
