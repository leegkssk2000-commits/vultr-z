...
BRANCH = "q4r3-team-advisor-r31-team-sgrade-gap-audit-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r31_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r31_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(wt / "tools/run_q4r3_team_advisor_r31_team_sgrade_gap_audit.py"),
            "--root", str(root),
            "--worktree", str(wt),
        ])
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r31_team_sgrade_gap_audit/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r31_team_sgrade_gap_audit_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R3.1 Team S-grade gap audit evidence",
            ], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)

        print("Q4R3_TEAM_ADVISOR_R31_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R3.1")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r31_team_sgrade_gap_audit_latest.json")
        passed = True
        return 0
    finally:
