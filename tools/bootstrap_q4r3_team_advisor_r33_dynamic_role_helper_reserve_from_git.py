...
BRANCH = "q4r3-team-advisor-r33-dynamic-role-helper-reserve-v1"
UNITS = {
...
def main() -> int:
    ...
    wt = Path(f"/tmp/q4r3_team_advisor_r33_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r33_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(wt / "tools/run_q4r3_team_advisor_r33_dynamic_role_helper_reserve.py"),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r33_dynamic_role_helper_reserve/status_latest.json"
        evidence = wt / "evidence/q4r3_team_advisor_r33_dynamic_role_helper_reserve_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=wt).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R3.3 dynamic role evidence",
            ], cwd=wt)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=wt)
        print("Q4R3_TEAM_ADVISOR_R33_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R3.3")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r33_dynamic_role_helper_reserve_latest.json")
        passed = True
        return 0
    finally:
