...
BRANCH = "q4r3-team-advisor-r44-lico-execution-cost-realistic-fill-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r44_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r44_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r44_lico_execution_cost_realistic_fill.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r44_lico_execution_cost_realistic_fill/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r44_lico_execution_cost_realistic_fill_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R4.4 Lico execution cost fill evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R44_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R4.4")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r44_lico_execution_cost_realistic_fill_latest.json")
        passed = True
        return 0
    finally:
