...
BRANCH = "q4r3-team-advisor-r45-lico-fee-funding-liquidation-stress-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r45_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r45_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r45_lico_fee_funding_liquidation_stress.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r45_lico_fee_funding_liquidation_stress/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r45_lico_fee_funding_liquidation_stress_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R4.5 Lico risk cost stress evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R45_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R4.5")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r45_lico_fee_funding_liquidation_stress_latest.json")
        passed = True
        return 0
    finally:
