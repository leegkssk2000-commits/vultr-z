...
BRANCH = "q4r3-team-advisor-r46-lico-team-lineage-calibration-sgrade-lock-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r46_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r46_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r46_lico_team_lineage_calibration_sgrade.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r46_lico_team_lineage_calibration_sgrade/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r46_lico_team_lineage_calibration_sgrade_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R4.6 Lico team lineage calibration S-grade evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R46_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R4.6")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r46_lico_team_lineage_calibration_sgrade_latest.json")
        passed = True
        return 0
    finally:
