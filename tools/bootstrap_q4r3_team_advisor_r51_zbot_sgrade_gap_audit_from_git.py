...
BRANCH = "q4r3-team-advisor-r51-zbot-sgrade-gap-audit-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r51_{args.sha[:12]}")
    ...
    before = unit_pids()
    with tempfile.NamedTemporaryFile(prefix="q4r3_r51_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r51_zbot_sgrade_gap_audit.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r51_zbot_sgrade_gap_audit/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r51_zbot_sgrade_gap_audit_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R5.1 ZBot S-grade gap audit evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R51_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R5.1")
        for name, value in after.items():
            print(f"{name}={value}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r51_zbot_sgrade_gap_audit_latest.json")
        passed = True
        return 0
    finally:
