...
BRANCH = "q4r3-team-advisor-r52-zbot-canonical-provider-policy-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r52_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r52_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r52_zbot_canonical_provider_policy.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r52_zbot_canonical_provider_policy/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r52_zbot_canonical_provider_policy_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R5.2 ZBot canonical provider policy evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)

        print("Q4R3_TEAM_ADVISOR_R52_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R5.2")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print(f"HOST_ZBOT_SHA256={after_host_zbot}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r52_zbot_canonical_provider_policy_latest.json")
        passed = True
        return 0
    finally:
