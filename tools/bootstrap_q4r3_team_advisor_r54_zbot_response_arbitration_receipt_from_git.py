...
BRANCH = "q4r3-team-advisor-r54-zbot-response-arbitration-receipt-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r54_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r54_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r54_zbot_response_arbitration_receipt.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r54_zbot_response_arbitration_receipt/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r54_zbot_response_arbitration_receipt_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R5.4 ZBot response arbitration receipt evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)
        print("Q4R3_TEAM_ADVISOR_R54_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R5.4")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print(f"HOST_ZBOT_SHA256={after_host_zbot}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r54_zbot_response_arbitration_receipt_latest.json")
        passed = True
        return 0
    finally:
