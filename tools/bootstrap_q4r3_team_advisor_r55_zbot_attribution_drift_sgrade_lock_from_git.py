...
BRANCH = "q4r3-team-advisor-r55-zbot-attribution-drift-sgrade-lock-v1"
UNITS = {
...
def main() -> int:
    ...
    worktree = Path(f"/tmp/q4r3_team_advisor_r55_{args.sha[:12]}")
    ...
    with tempfile.NamedTemporaryFile(prefix="q4r3_r55_ledger_", delete=False) as handle:
    ...
    try:
        ...
        run([
            python_bin(root),
            str(worktree / "tools/run_q4r3_team_advisor_r55_zbot_attribution_drift_sgrade_lock.py"),
            "--root", str(root),
        ...
        status = root / "runtime/exact25_edge_v1/team_advisor_r55_zbot_attribution_drift_sgrade_lock/status_latest.json"
        evidence = worktree / "evidence/q4r3_team_advisor_r55_zbot_attribution_drift_sgrade_lock_latest.json"
        ...
        if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=worktree).returncode != 0:
            run([
                ...
                "commit", "-m", "Record R5.5 ZBot attribution drift S-grade evidence",
            ], cwd=worktree)
            run(["git", "push", "origin", f"HEAD:refs/heads/{args.branch}"], cwd=worktree)
        print("Q4R3_TEAM_ADVISOR_R55_BOOTSTRAP_PASS")
        print("OFFICIAL_STAGE=R5.5")
        for name, value in after_pids.items():
            print(f"{name}={value}")
        print(f"HOST_ZBOT_SHA256={after_host_zbot}")
        print("EVIDENCE=evidence/q4r3_team_advisor_r55_zbot_attribution_drift_sgrade_lock_latest.json")
        passed = True
        return 0
    finally:
