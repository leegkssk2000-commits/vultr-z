...
SHA="${Q4R3_SGRADE_SHA:?Q4R3_SGRADE_SHA_REQUIRED}"
TARGET_BRANCH="${Q4R3_TARGET_BRANCH:-q4r3-team-advisor-sgrade-readiness-v1}"
WT="/tmp/q4r3_team_advisor_sgrade_${SHA:0:12}"
...
Q4R3_ROOT="$ROOT" Q4R3_WORKTREE="$WT" Q4R3_TARGET_BRANCH="$TARGET_BRANCH" \
  bash "$WT/tools/run_q4r3_team_advisor_sgrade_readiness_audit.sh"

echo Q4R3_TEAM_ADVISOR_SGRADE_READINESS_BOOTSTRAP_PASS
