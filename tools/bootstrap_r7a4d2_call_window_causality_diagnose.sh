...
printf '%s\n' \
  'R7A4D2_CALL_WINDOW_CAUSALITY_START' \
  'MODE=READ_ONLY_FLAT_ENTRY_TERMINAL_ADD_CHAIN_AND_TARGETED_REPLAY_DIAGNOSE' \
  'HISTORICAL_SIMULATION_3600_REEXECUTION_ALLOWED=false' \
  'EVENT_REPLAY_2880_ALLOWED=false' \
  'TARGETED_REPLAY_SCENARIO_LIMIT=120' \
...
if [[ -z "$SHA" ]] || ! git -C "$ROOT" cat-file -e "$SHA^{commit}" 2>/dev/null; then
  ...
  echo 'R7A4D2_CALL_WINDOW_CAUSALITY_COMPLETE'
  echo 'RC=2'
  exit 2
fi

TMP="$(mktemp -d /tmp/r7a4d2_call_window.XXXXXX)" || exit 2
for path in \
  tools/r7a4d2_call_window_causality_diagnose.py \
  tools/r7a4d_historical_simulation_3600.py \
  tests/test_r7a4d2_call_window_causality_diagnose.py \
...
if ! python3 -m py_compile \
  "$TMP/tools/r7a4d2_call_window_causality_diagnose.py" \
...
if ! PYTHONPATH="$TMP:$ROOT" python3 -m pytest -q \
  "$TMP/tests/test_r7a4d2_call_window_causality_diagnose.py"; then
  echo 'STATE=HOLD'
  echo 'BLOCKER_COUNT=1'
