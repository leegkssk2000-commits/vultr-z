...
printf '%s\n' \
  'R7A4D2_ENTRY_CHAIN_MINIMAL_PATCH_START' \
  'MODE=TEMPORARY_PATCH_AND_TARGETED_REPLAY_VERIFY' \
  'INDICATOR_PREROLL_BARS=320' \
  'EVALUATION_BARS=320' \
  'TARGETED_REPLAY_SCENARIOS=120' \
...
if [[ -z "$SHA" ]] || ! git -C "$ROOT" cat-file -e "$SHA^{commit}" 2>/dev/null; then
  echo 'STATE=HOLD_ENTRY_CHAIN_MINIMAL_PATCH'
...
TMP="$(mktemp -d /tmp/r7a4d2-entry-chain.XXXXXX)" || exit 2
for path in \
  tools/r7a4d_historical_simulation_3600.py \
  tools/r7a4d2_entry_chain_minimal_patch.py \
  tools/r7a4d2_entry_chain_minimal_patch_verify.py \
  tests/test_r7a4d2_entry_chain_minimal_patch.py \
...
if ! python3 -m py_compile \
  ...
  "$TMP/tools/r7a4d2_entry_chain_minimal_patch_verify.py"; then
  echo 'STATE=HOLD_ENTRY_CHAIN_MINIMAL_PATCH'
  echo 'BLOCKER_COUNT=1'
...
if ! python3 "$TMP/tools/r7a4d2_entry_chain_minimal_patch.py" \
  ...
  --output "$TMP/tools/r7a4d2_patched_historical_simulation_3600.py"; then
  echo 'STATE=HOLD_ENTRY_CHAIN_MINIMAL_PATCH'
  echo 'BLOCKER_COUNT=1'
  echo 'BLOCKERS=["PATCH_BUILD_FAILED"]'
  echo 'RC=2'
  exit 2
fi

if ! PYTHONPATH="$TMP:$ROOT" python3 -m pytest -q \
  "$TMP/tests/test_r7a4d2_entry_chain_minimal_patch.py"; then
  echo 'STATE=HOLD_ENTRY_CHAIN_MINIMAL_PATCH'
  echo 'BLOCKER_COUNT=1'
...
PYTHONPATH="$ROOT:$TMP" python3 "$TMP/tools/r7a4d2_entry_chain_minimal_patch_verify.py" \
  --root "$ROOT" \
  --runner "$TMP/tools/r7a4d2_patched_historical_simulation_3600.py" \
  --contract "$TMP/backend/contracts/ZOS_R7A4D_HISTORICAL_SIMULATION_3600_v1.json"
RC=$?

echo 'R7A4D2_ENTRY_CHAIN_MINIMAL_PATCH_COMPLETE'
echo "RC=$RC"
exit "$RC"
