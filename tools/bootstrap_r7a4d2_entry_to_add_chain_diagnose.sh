...
printf '%s\n' \
  'R7A4D2_ENTRY_TO_ADD_CHAIN_DIAGNOSE_START' \
  'MODE=READ_ONLY_SOURCE_AST_REGISTRY_ROLE_AND_PAIRED_STATE_DIAGNOSE' \
...
if [[ -z "$SHA" ]] || ! git -C "$ROOT" cat-file -e "$SHA^{commit}" 2>/dev/null; then
  ...
  echo 'R7A4D2_ENTRY_TO_ADD_CHAIN_DIAGNOSE_COMPLETE'
  echo 'RC=2'
  exit 2
fi

TMP="$(mktemp -d /tmp/r7a4d2_entry_to_add.XXXXXX)" || exit 2
for path in \
  tools/r7a4d2_entry_to_add_chain_diagnose.py \
  tools/r7a4d_historical_simulation_3600.py \
  tests/test_r7a4d2_entry_to_add_chain_diagnose.py \
...
if ! python3 -m py_compile \
  "$TMP/tools/r7a4d2_entry_to_add_chain_diagnose.py" \
...
if ! PYTHONPATH="$TMP:$ROOT" python3 -m pytest -q \
  "$TMP/tests/test_r7a4d2_entry_to_add_chain_diagnose.py"; then
  echo 'STATE=HOLD'
  echo 'BLOCKER_COUNT=1'
