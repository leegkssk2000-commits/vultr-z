...
printf '%s\n' \
  'R7A4D2_INCREMENTAL_DEFECT3_CONSECUTIVE_LOSS_CAUSALITY_AUDIT_START' \
  'MODE=READ_ONLY_ACTIVE_PARENT_CONSECUTIVE_LOSS_CAUSALITY_AND_SPLIT_PERSISTENCE_AUDIT' \
  ...
  'CONSECUTIVE_LOSS_LENGTH_ALONE_IS_NOT_REPAIR_EVIDENCE=true' \
  'DISCOVERY_VALIDATION_STREAK_PERSISTENCE_REQUIRED=true' \
  'LOSS_CAUSE_DECOMPOSITION_REQUIRED=REENTRY_CHURN,COST_EROSION,NO_FAVORABLE_EXCURSION,EXIT_CAPTURE_FAILURE,FAST_STOP_VOLATILITY,TIMEOUT_DRIFT' \
  'MFE_MAE_RECOMPUTE_FROM_FROZEN_OHLCV=true' \
  'STREAK_RANDOMNESS_PERMUTATION_CHECK=true' \
  'MAX_ACTIVE_REPAIR_LANES=3' \
  'ONE_DEFECT_PER_LANE=true' \
...
if [[ -z "$SHA" ]] || ! git -C "$ROOT" cat-file -e "$SHA^{commit}" 2>/dev/null; then
  echo 'STATE=HOLD_INCREMENTAL_DEFECT3_CONSECUTIVE_LOSS_CAUSALITY_AUDIT_INPUT'
...
TMP="$(mktemp -d /tmp/r7a4d2-defect3-streak-audit.XXXXXX)" || exit 2
AUDIT="$TMP/r7a4d2_incremental_defect3_consecutive_loss_causality_audit.py"
RAW="$TMP/r7a4d2_short_raw_geometry_and_simple_benchmark_execution.py"

if ! git -C "$ROOT" show "$SHA:tools/r7a4d2_incremental_defect3_consecutive_loss_causality_audit.py" > "$AUDIT"; then
  echo 'STATE=HOLD_INCREMENTAL_DEFECT3_CONSECUTIVE_LOSS_CAUSALITY_AUDIT_INPUT'
  echo 'BLOCKER_COUNT=1'
...
if ! python3 -m py_compile "$AUDIT" "$RAW"; then
  echo 'STATE=HOLD_INCREMENTAL_DEFECT3_CONSECUTIVE_LOSS_CAUSALITY_AUDIT_INPUT'
  echo 'BLOCKER_COUNT=1'
...
echo 'R7A4D2_INCREMENTAL_DEFECT3_CONSECUTIVE_LOSS_CAUSALITY_AUDIT_BOOTSTRAP_COMPLETE'
echo "RC=$RC"
exit "$RC"
