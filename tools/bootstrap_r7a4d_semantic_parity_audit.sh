...
printf '%s\n' \
  'R7A4D_SEMANTIC_AUDIT_START' \
  'MODE=READ_ONLY_CANONICAL_ADAPTER_VS_LEGACY_SIGNAL_PARITY' \
...
if [[ -z "$SHA" ]] || ! git -C "$ROOT" cat-file -e "$SHA^{commit}" 2>/dev/null; then
  ...
  echo 'R7A4D_SEMANTIC_AUDIT_COMPLETE'
  echo 'RC=2'
  exit 2
fi

TMP="$(mktemp -d /tmp/r7a4d_semantic.XXXXXX)" || exit 2
for path in \
  tools/r7a4d_semantic_parity_audit.py \
  tools/r7a4d_historical_simulation_3600.py \
  tests/test_r7a4d_semantic_parity_audit.py \
...
if ! python3 -m py_compile \
  "$TMP/tools/r7a4d_semantic_parity_audit.py" \
...
if ! PYTHONPATH="$TMP:$ROOT" python3 -m pytest -q "$TMP/tests/test_r7a4d_semantic_parity_audit.py"; then
  echo 'STATE=HOLD'
  echo 'BLOCKER_COUNT=1'
...
PYTHONPATH="$ROOT:$TMP" python3 "$TMP/tools/r7a4d_semantic_parity_audit.py" \
  --root "$ROOT" \
