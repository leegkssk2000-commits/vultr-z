...
def main() -> int:
    ...
    if receipt.get("state") != "PASS" or receipt.get("blocker_count") != 0:
        blockers.append("QUARANTINE_RECEIPT_NOT_PASS")
    for key in ("target_count", "original_absent_count", "backup_verified_count", "isolated_verified_count"):
        if receipt.get(key) != 2:
            blockers.append(f"RECEIPT_{key.upper()}_INVALID")
    result = {
        ...
        "receipt_verified": receipt.get("state") == "PASS",
