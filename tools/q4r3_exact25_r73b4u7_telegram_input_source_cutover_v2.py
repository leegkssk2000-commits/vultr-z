...
ERROR_TOKENS = ("Traceback", "ERROR", "Exception", "NameError", "TypeError", "AttributeError", "SyntaxError")
...
def capture_path_state(path: Path, backup_dir: Path, name: str) -> dict[str, Any]:
    record: dict[str, Any] = {"path": str(path), "kind": "absent"}
    if path.is_symlink():
        record.update({"kind": "symlink", "target": os.readlink(path)})
    elif path.exists():
        if not path.is_file():
            raise RuntimeError(f"NON_FILE_PATH:{path}")
        backup = backup_dir / f"{name}.bak"
...
def zero_payload() -> dict[str, Any]:
    return {
        ...
        "candidate": "0", "candidate_count": "0",
        "admitted": "0", "admitted_count": "0",
        "open": "0", "open_count": "0",
        "closed": "0", "closed_count": "0",
        "pnl": "0", "pnl_r": "0", "net_r": "0",
        "shadow_open": "0", "paper_open": "0", "live_open": "0",
        "current": {},
        "last_close": "none", "last_closed": "none",
        "state": "HOLD_ZERO_EPOCH_PENDING", "action": "hold",
        "recent_rows": "0", "rows": "0",
        "last12": "0", "last12_r": "0",
        "wr": "0", "wr_pct": "0", "winrate": "0", "winrate_pct": "0", "win_rate": "0",
        "ev": "0", "ev_r": "0", "expectancy": "0", "expectancy_r": "0",
        "order": "blocked", "order_authority": "blocked",
        "exec": "none", "execution_authority": "none",
        "runtime_active": False, "formal_ledger_bound": False,
        "src": "telegram_zero_compat_input.json", "source": "telegram_zero_compat_input.json"
    }
...
def main() -> int:
    ...
    shadow_snapshot_path = Path(contract["shadow_snapshot"])
    ...
    for path in (diagnosis_path, source_path, canonical_path, shadow_snapshot_path):
        if not path.is_file():
    ...
    shadow_snapshot = read_json(shadow_snapshot_path)
    ...
    if shadow_snapshot.get("runtime_active") is not False or shadow_snapshot.get("formal_ledger_bound") is not False:
    ...
    backup_dir = backup_root / str(time.time_ns())
    backup_dir.mkdir(parents=True, exist_ok=True)
    canonical_record = capture_path_state(canonical_path, backup_dir, "canonical_output")
    compat_record = capture_path_state(compat_path, backup_dir, "compat_input")
    secondary_records = [capture_path_state(path, backup_dir, f"secondary_{index}") for index, path in enumerate(secondary_paths)]
    ...
    try:
        ...
        if stopped.returncode != 0:
            raise RuntimeError("TELEGRAM_STOP_FAILED:" + stopped.stderr[-300:])
        ...
        if started.returncode != 0:
            raise RuntimeError("TELEGRAM_START_FAILED:" + started.stderr[-300:])
        time.sleep(3)

        active = run(["systemctl", "is-active", unit]).stdout.strip()
        if active != "active":
            raise RuntimeError("TELEGRAM_UNIT_NOT_ACTIVE:" + active)
        compiled = run(["python3", "-m", "py_compile", str(source_path)])
        if compiled.returncode != 0:
            raise RuntimeError("TELEGRAM_COMPILE_FAILED:" + compiled.stderr[-300:])
        if sha256(source_path) != source_before:
            raise RuntimeError("TELEGRAM_SOURCE_CHANGED")

        alias_failures = [str(path) for path in secondary_paths if not path.is_symlink() or path.resolve(strict=False) != compat_path.resolve(strict=False)]
        if alias_failures:
        ...
        last_close = str(first(live, "last_close", "last_closed")).strip().lower()
        if last_close not in {"none", "", "{}"}:
            raise RuntimeError("LAST_CLOSE_RESIDUE_REMAINS:" + last_close)

        journal = run(["journalctl", "-u", unit, "--since", f"@{started_at}", "--no-pager", "-o", "cat"]).stdout
        runtime_errors = [line for line in journal.splitlines() if any(token in line for token in ERROR_TOKENS)]
        if runtime_errors:
        ...
        shadow_after = read_json(shadow_snapshot_path)
        ...
        result = {
            ...
            "telegram_compile_ok": True, "telegram_unit_active": True,
            "telegram_runtime_error_count": 0,
            "legacy_input_path_count": len(secondary_paths),
            "legacy_input_cutover_count": len(secondary_paths),
            "canonical_closed_count": 0, "canonical_recent_rows": 0,
            "canonical_last12_r": 0.0, "canonical_winrate_pct": 0.0,
            "canonical_ev_r": 0.0, "canonical_pnl_r": 0.0,
        ...
        for record in secondary_records:
            restore_path(record)
        restore_path(compat_record)
        restore_path(canonical_record)
        run(["systemctl", "start", unit])
        result = {
            "schema": "q4r3_exact25_r73b4u7_telegram_input_source_cutover_status_v1",
            "state": "HOLD", "blockers": [str(exc)], "blocker_count": 1,
            "mutation_count": len(mutations), "mutations": mutations,
            "rollback_performed": True, "telegram_source_change_count": 0,
            "runtime_active": False, "next_stage": "R7.3B4U7_DIAGNOSE"
        }
        atomic_json(args.status, result)
        print(json.dumps(result, sort_keys=True))
        return 2
