...
def project(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    all_blocked_count = 0
    unassigned_blocked_count = 0
    unassigned_blocked_methods: set[str] = set()
    ...
    for row in events:
        ...
        if event_type == "skill_blocked":
            all_blocked_count += 1
            if method_id in PROFILES:
                blocked[method_id] += 1
            else:
                unassigned_blocked_count += 1
                unassigned_blocked_methods.add(method_id or "<missing>")
            continue
    ...
    profile_blocked_count = sum(row["blocked_count"] for row in rows)
    return {
        ...
        "total_blocked_count": all_blocked_count,
        "profile_blocked_count": profile_blocked_count,
        "unassigned_blocked_count": unassigned_blocked_count,
        "unassigned_blocked_method_ids": sorted(unassigned_blocked_methods),
...
def run(args: argparse.Namespace) -> int:
    ...
    if projection["profile_blocked_count"] + projection["unassigned_blocked_count"] != projection["total_blocked_count"]:
        issues.append({"code": "BLOCKED_COUNT_ACCOUNTING_MISMATCH", "severity": "C", "detail": str(projection), "source": "projection"})

    state = "HOLD" if any(row["severity"] == "C" for row in issues) else "PASS"
    if state == "HOLD":
        verdict = "SIX_PROFILE_PROJECTION_CRITICAL_GAP"
    elif projection["total_trigger_count"] == 0 and projection["total_blocked_count"] > 0:
        verdict = "SIX_PROFILE_PROJECTION_HEALTHY_BLOCKED_CONTEXT_PENDING"
    ...
    status = {
        ...
        "profile_blocked_count": projection["profile_blocked_count"],
        "unassigned_blocked_count": projection["unassigned_blocked_count"],
        "unassigned_blocked_method_ids": projection["unassigned_blocked_method_ids"],
