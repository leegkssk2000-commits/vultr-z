...
def validate(r54_path: Path, contract_path: Path) -> dict[str, Any]:
    ...
    if contract.get("surface_count", {}) != {"prior_ready": 22, "closed_now": 2, "total_ready": 24, "total": 24}:
    ...
    attribution_result = attribution.evaluate_attribution(
        sample_outcomes(), expected_provider_ids=PROVIDERS, now_ms=10000,
        policy=attribution.AttributionPolicy(3, 1000, 1000, 0.10, "sheets:zbot:validator:attribution"),
    )
    drift_result = drift.evaluate_quality_drift(
        snapshots(False), snapshots(True), expected_provider_ids=PROVIDERS, now_ms=10000,
        policy=drift.DriftPolicy(30, 1000, 0.10, 0.10, 0.10, 0.05, 1.50, "sheets:zbot:validator:drift"),
    )
